...
## 更新摘要
**所做更改**
- 更新架构概述，反映从传统架构向 Next.js 15 + TailwindCSS v4 的 CSS-first 方法论迁移
- 更新项目结构章节，强调新的开发工作流程和配置管理
- 更新依赖关系分析，包含新的 CSS-in-JS 和构建工具链
- 更新性能考量，涵盖 TailwindCSS v4 的新特性和优化策略
- 更新故障排查指南，包含新的配置相关问题
...
**更新** 投资组合管理系统已从传统架构全面迁移到 Next.js 15 + TailwindCSS v4 的 CSS-first 方法论，采用现代化的开发工作流程和配置管理策略。

## 项目结构
前端采用 Next.js App Router 的目录约定进行页面与布局组织，核心入口位于应用根布局与全局提供者，页面按功能域分层，组件按职责拆分至桌面端/移动端/共享 UI 层，并通过 Zustand 和 React Query 实现状态与数据流管理。新的架构采用 CSS-first 方法论，通过 TailwindCSS v4 实现高度可定制的样式系统。
...
V["TailwindCSS v4<br/>CSS-first 方法论"] --> W["原子化样式系统"]
X["PostCSS 配置<br/>postcss.config.js"] --> Y["构建优化与插件"]
Z["Next.js 15<br/>App Router"] --> AA["路由与页面组织"]
```
...
- [frontend/postcss.config.js:1-20](file://frontend/postcss.config.js#L1-L20)
...
- UI 组件库
  - 基于 TailwindCSS v4 的 CSS-first 方法论，提供原子化样式系统和高度可定制的主题。
  - 新增 AlertDialog 对话框组件，支持确认/取消操作的模态交互。
  - 新增 DatePicker 日期选择组件，基于 Popover 和 Calendar 实现，支持中文本地化。

**更新** 采用 TailwindCSS v4 的 CSS-first 方法论，通过原子化样式系统实现更灵活的样式定制和更好的性能表现。
...
- 样式基础
  - TailwindCSS v4 配置扩展主题色与圆角变量，确保组件风格一致。

**更新** TailwindCSS v4 提供了更强大的原子化样式系统和更好的性能优化。
...
- 与设计系统的耦合：基于 TailwindCSS v4 与原子化样式系统，降低对特定库的绑定风险。

**更新** 采用 TailwindCSS v4 的 CSS-first 方法论，通过原子化样式实现更灵活的设计系统。
...
## 依赖关系分析
- 外部依赖
  - Next.js 15、React、Axios、@tanstack/react-query、zustand、lucide-react、tailwindcss v4 等。
  - 新增 TailwindCSS v4 的 CSS-first 方法论和 PostCSS 配置。
...
- 代码分割与构建
  - Next.js 15 默认按路由进行代码分割；页面级懒加载与组件级懒加载可进一步优化首屏。
  - 新增的 UI 组件按需加载，避免不必要的包体积。
- 样式与渲染
  - TailwindCSS v4 基于内容扫描，生产构建可移除未使用样式；组件尽量使用原子类，减少自定义样式体积。
  - CSS-first 方法论通过原子化样式实现更好的性能表现。
- 网络代理
  - 开发阶段通过 next.config.js 重写 /api/* 到后端，减少跨域问题与调试成本。
- 构建优化
  - PostCSS 配置提供构建时的样式优化和插件支持，提升构建性能。

**更新** TailwindCSS v4 引入了新的 CSS-first 方法论，通过原子化样式系统实现更优的性能表现和更好的开发体验。
...
- TailwindCSS v4 样式问题
  - 现象：样式不生效或构建错误。
  - 排查：确认 PostCSS 配置正确；检查内容扫描路径；验证原子化样式的使用方式。
- Next.js 15 路由问题
  - 现象：页面无法访问或路由错误。
  - 排查：确认 App Router 配置；检查路由文件命名和结构；验证动态路由参数。

**更新** 新增 TailwindCSS v4 和 Next.js 15 相关的故障排查指南。
...
## 结论
InvestRing 前端以 Next.js 15 App Router 为基础，结合 Zustand 与 React Query 实现清晰的状态与数据流管理，辅以 TailwindCSS v4 的 CSS-first 方法论构建一致的 UI 体系。通过主布局与多套导航组件实现桌面/移动端一体化体验，配合持久化与缓存策略提升性能与可用性。

**更新** 新架构采用 TailwindCSS v4 的 CSS-first 方法论，通过原子化样式系统实现更灵活的设计和更好的性能表现。新的开发工作流程提供了更高效的开发体验和更可靠的构建过程。

后续可在以下方面持续演进：细化路由权限控制、增强错误边界与监控、引入组件测试与文档化、扩展 UI 组件库以支持更多业务场景、优化 TailwindCSS v4 的主题定制和性能优化。

## 附录
- 关键配置参考
  - next.config.js：开发代理与严格模式配置。
  - postcss.config.js：PostCSS 插件配置与构建优化。
- 依赖版本
  - Next.js 15、React、Axios、React Query、Zustand、TailwindCSS v4 等版本见 package.json。
  - 新增依赖：@radix-ui/react-alert-dialog、@radix-ui/react-dialog、@radix-ui/react-popover、date-fns、react-day-picker。

**更新** 新增 TailwindCSS v4 和 PostCSS 相关配置说明。
