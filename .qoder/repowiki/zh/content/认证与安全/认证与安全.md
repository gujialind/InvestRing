...
## 更新摘要
**变更内容**   
- 更新了认证错误处理机制，401响应时统一调用authStore.logout()清除所有三个存储位置
- 密码重置接口从POST方法改为PUT方法
- 增强了前端认证状态管理的清理逻辑
...
- 请求拦截
  ...
  - **更新** 401 统一处理：调用 authStore.logout() 清空所有三个存储位置（localStorage、Cookie、内存状态），然后跳转登录页。
- Token 刷新
...
**图表来源**
- [frontend/src/stores/authStore.ts:37-51](file://frontend/src/stores/authStore.ts#L37-L51)
- [frontend/src/lib/api.ts:50-79](file://frontend/src/lib/api.ts#L50-79)
...
- 改密
  - **更新** 接口方法已从 POST 改为 PUT 方法，提升 RESTful 规范性。
...
participant SEC as "安全工具"
participant DB as "数据库"
FE->>AX : "PUT /auth/password 或 POST /auth/logout"
...
- 路由依赖
  ...
  - /auth/password：**更新** 使用 PUT 方法而非 POST，依赖 get_current_user、权限校验、bcrypt 密码校验、HS256 Token 黑名单。
- 依赖注入
...
- 401 未授权
  ...
  - **更新** 确认 401 响应时是否正确触发 authStore.logout() 清理所有存储位置。
- 403 禁止访问
...
## 结论
InvestRing 的认证与安全体系以 HS256 JWT 为核心，结合 bcrypt 密码哈希、Token 黑名单与账户锁定机制，实现了基础而有效的无状态认证。**最新更新**包括统一的 401 错误处理机制和更规范的密码重置接口方法。前端通过 Axios 拦截器与 Zustand 状态管理，提供了简洁一致的认证体验。建议在生产环境中强化密钥管理、启用 HTTPS 与安全 Cookie、并考虑引入更完善的 Token 刷新与多设备会话管理策略。
...
- 前端本地存储风险
  - **更新** 利用统一的 401 处理机制确保所有存储位置被正确清理，强制使用 HttpOnly Cookie（建议）与最小权限存储，定期清理本地缓存。