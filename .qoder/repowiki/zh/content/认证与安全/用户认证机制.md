...
## 更新摘要
**变更内容**   
- 更新了登出功能，增强了 Token 失效处理机制
- 完善了 POST /api/auth/logout 端点的实现，确保 Token 被加入黑名单
- 改进了登出事件的安全日志记录功能
- 增强了认证中间件的 Token 验证逻辑
...
**更新** 本次更新重点增强了登出功能的安全性，通过完善的 Token 黑名单机制和详细的登出日志记录，提升了系统的安全防护能力。
...
- **增强** 登出接口：从请求头解析 Bearer Token 并加入黑名单，记录登出日志，确保 Token 完全失效。
...
**更新** 登出功能现已完善，能够正确地将 Token 加入黑名单并记录详细的登出日志，提升了系统的安全性。
...
## 架构总览
下图展示认证端到端流程：前端发起登录请求，后端验证、记录日志、签发 Token；后续请求由前端携带 Token，后端中间件完成鉴权与授权；登出时 Token 被加入黑名单，确保安全性。
...
### 增强的登出与 Token 黑名单机制
- **新增** 登出接口：从 Authorization 头提取 Bearer Token，将其加入黑名单，记录详细的登出日志。
- **改进** 中间件：get_current_user 在每次受保护请求中检查 Token 是否在黑名单内，若在则拒绝访问。
- **安全增强** 登出事件记录：包含用户信息、IP 地址、User-Agent 等详细信息，便于安全审计。
...
API->>DB : "记录详细登出日志"
API-->>FE : "{message}"
Note over API,DB : 登出后立即使所有该用户的 Token 失效
```

**更新** 登出功能现已完善，能够正确地将 Token 加入黑名单并记录详细的登出日志，确保用户登出后无法继续使用任何已签发的 Token。
...
- 最佳实践
  ...
  - 定期轮换密钥并监控登录日志与审计日志。
  - **新增** 登出时主动调用登出接口，确保 Token 被服务器端标记为失效。

**更新** 前端集成现已支持完整的登出流程，包括调用服务器端登出接口和清理本地状态。
...
- **新增** 登出操作性能：Token 黑名单检查为 O(1) 复杂度，登出日志写入为异步操作，对主流程影响极小。

**更新** 登出功能的性能优化确保了在高并发场景下的稳定表现。
...
- **新增** 登出后仍能访问
  - 检查登出接口是否正确调用；确认 Token 已被加入黑名单；验证中间件的黑名单检查逻辑。
...
## 结论
InvestRing 的认证体系以 HS256 JWT 为核心，结合 bcrypt 密码哈希、内存级失败锁定与黑名单机制，实现了基础而实用的安全保障。**本次更新显著增强了登出功能的安全性**，通过完善的 Token 黑名单机制和详细的登出日志记录，确保了用户登出后的会话安全。前端通过统一拦截器与状态管理简化了集成，配合受保护路由与中间件，形成闭环的认证与授权链路。生产部署建议增强共享缓存、启用 HTTPS 与 CSRF 保护，并完善日志与审计能力。
...
- **新增** 响应
  - 成功：{message}
  - 失败：401（未提供 Token/Token 无效）

**更新** 新增了登出 API 的详细文档，包括请求格式和响应说明。
...
- 调用方式：通过 authApi.login、authApi.changePassword、authApi.getCurrentUser、authApi.logout。
- 状态与守卫：useAuthStore、useLogin、useLogout、useCurrentUser、useAuthGuard。

**更新** 新增了登出相关的类型定义和调用方式。
