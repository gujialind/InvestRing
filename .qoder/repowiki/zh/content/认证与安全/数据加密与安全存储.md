...
- 数据库：连接池（QueuePool）、MySQL InnoDB 引擎、模型定义（含密码哈希字段）
...
subgraph "数据库"
DB["database.py<br/>QueuePool连接池/InnoDB/会话"]
...
- 数据库存储：密码字段采用 bcrypt 哈希；MySQL InnoDB 引擎配合 QueuePool 连接池提升并发性能
...
- 数据库并发：MySQL InnoDB 引擎提供行级锁与 MVCC 并发控制，配合 QueuePool 连接池提升读写并发性能
...
- 数据库并发：MySQL InnoDB 引擎提供行级锁与 MVCC，配合 QueuePool 连接池显著提升读写并发性能
...
- 登出后仍可访问
  ...
  - 检查依赖注入中黑名单检查逻辑是否生效
- 数据库并发问题
  - 确认 MySQL InnoDB 引擎连接池配置（QueuePool pool_size、max_overflow、pool_pre_ping）
  - 避免在高并发导入场景下同时执行定时任务
...
## 结论
InvestRing 在密码存储、会话管理与数据库并发方面采用了成熟且平衡的安全实践：bcrypt 哈希、HS256 JWT、黑名单与失败追踪、MySQL InnoDB 引擎与 QueuePool 连接池。建议在生产环境中进一步完善密钥轮换、传输加密与备份加密策略，以满足更严格的合规要求。
