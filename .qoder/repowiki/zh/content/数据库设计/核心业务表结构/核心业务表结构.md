...
## 更新摘要
**变更内容**   
- product_code字段从String(10)扩展到String(20)，支持更长的产品代码
- 新增两个虚拟产品IN_TRANSIT_BUY和IN_TRANSIT_SELL，用于处理在途交易
- 更新产品模型定义和相关服务逻辑
- 增强产品代码验证和业务规则
- 完善在途交易的完整业务流程支持
...
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [product_code字段扩展](#product_code字段扩展)
7. [虚拟产品机制](#虚拟产品机制)
8. [迁移改进与可靠性增强](#迁移改进与可靠性增强)
9. [依赖分析](#依赖分析)
10. [性能考量](#性能考量)
11. [故障排查指南](#故障排查指南)
12. [结论](#结论)
...
**更新** 本次更新重点实现了product_code字段的扩展和虚拟产品机制的引入，通过扩展product_code字段长度至20个字符并新增IN_TRANSIT_BUY和IN_TRANSIT_SELL两个虚拟产品，显著增强了系统在途交易处理能力。
...
- **新增** 支持虚拟产品IN_TRANSIT_BUY和IN_TRANSIT_SELL，用于处理在途交易
...
def upgrade():
    # 扩展product表code字段长度
    op.alter_column('product', 'code', 
                   existing_type=sa.String(10),
                   new_type=sa.String(20))
    
    # 同步更新相关外键约束
    # 确保所有引用product.code的外键都兼容新长度
```
...
VIRTUAL_PRODUCTS = {
    'IN_TRANSIT_BUY': {
        ...
        'type': 'virtual',
        'market': 'VIRTUAL',
        'description': '已申请但未确认的买入交易'
    },
    'IN_TRANSIT_SELL': {
        'name': '在途卖出', 
        'type': 'virtual',
        'market': 'VIRTUAL',
        'description': '已提交但未确认的卖出交易'
    }
}

def is_virtual_product(product_code: str) -> bool:
    """判断是否为虚拟产品"""
    return product_code in VIRTUAL_PRODUCTS
```
...
#### 虚拟产品初始化
```python
def initialize_virtual_products():
    """初始化虚拟产品数据"""
    virtual_products = [
        {'code': 'IN_TRANSIT_BUY', 'name': '在途买入', 'type': 'virtual'},
        {'code': 'IN_TRANSIT_SELL', 'name': '在途卖出', 'type': 'virtual'}
    ]
    
    for product in virtual_products:
        if not exists_in_database(product['code']):
            insert_virtual_product(product)
```
...
**更新** 最新的product_code字段扩展和虚拟产品机制进一步增强了系统的灵活性和可扩展性。通过扩展产品代码长度至20个字符并引入IN_TRANSIT_BUY和IN_TRANSIT_SELL虚拟产品，显著提升了系统在途交易处理能力。配合完善的迁移脚本和可靠性保障措施，确保了系统升级过程中的稳定性和向后兼容性。建议在生产环境中谨慎执行迁移脚本，做好充分的数据备份和回滚准备。
...
- **新增** 虚拟产品机制相关文件
  ...
  - 在途交易处理逻辑：[backend/app/services/product_service.py](file://backend/app/services/product_service.py)