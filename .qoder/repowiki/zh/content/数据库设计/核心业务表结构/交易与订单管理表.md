...
## 更新摘要
**所做更改**
- 新增transfer_group字段的非空约束说明，消除孤立交易的可能性
- 添加Alembic迁移0003_trade_transfer_group_not_null.py的详细分析
- 更新交易表结构字段定义，包含transfer_group字段
- 增强数据完整性约束说明
...
**最新更新**：交易表新增了transfer_group字段的非空约束，通过Alembic迁移确保数据完整性，消除了孤立交易的可能性。
...
- 字段与类型
  ...
  - product_code: 字符串，配合 market 作为复合外键 product.code+market
...
### Alembic迁移与数据完整性保障
- **迁移文件**：0003_trade_transfer_group_not_null.py
- **主要功能**：
...
## 结论
交易与订单管理表围绕"交易（trade）、申购赎回（subscription）、价格记录（price_record）"构建，通过产品（product）与组合（portfolio）等模型形成闭环。系统在交易确认、净值获取、可用资金/份额计算、状态流转等方面具备完善的业务规则与错误处理机制。**最新的transfer_group非空约束进一步增强数据完整性，消除了孤立交易的可能性，提升了系统的可靠性和可追溯性**。配合索引与实时计算策略，能够满足日常交易与订单管理的高可靠与高性能需求。
