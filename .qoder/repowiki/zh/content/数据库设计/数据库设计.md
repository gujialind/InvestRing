...
## 更新摘要
**变更内容**
- 增强 Alembic 迁移系统的幂等性保护机制，确保迁移脚本的可重复执行
- 改进 MySQL 8.4 兼容性，优化 Alembic 迁移过程中的数据库操作
- 新增 idempotency_cache 表用于支持幂等性缓存功能
- 完善迁移脚本的错误处理和回滚机制
- 优化数据库连接池配置以支持更高并发场景
...
**最新更新**：支持平台间现金转移功能，通过 `transfer_group` 字段关联配对交易；申购赎回记录按平台拆分追踪；持仓快照按平台维度生成；份额变动事件表字段规范化，`event_date` 重命名为更具语义的 `ex_date`；**新增 investor_holding 表派生字段支持，提供市值、成本和利润的直接查询能力**；**标准化份额数量为精确的2位小数，确保财务计算的准确性和一致性**；**增强 Alembic 迁移系统的幂等性保护和 MySQL 8.4 兼容性**。
...
- 迁移系统：backend/alembic 目录包含 Alembic 配置文件、环境设置和迁移脚本，支持幂等性保护和 MySQL 8.4 兼容性。
...
- **Alembic 迁移增强**：**新增幂等性保护机制和 MySQL 8.4 兼容性支持，提升迁移可靠性**
...
IDEMPOTENCY_CACHE ||..|| IDEMPOTENCY_CACHE : "key唯一性"
```
...
- **新增功能**：支持幂等性保护的缓存机制，确保迁移脚本和其他关键操作的重复执行安全性
...
- ORM 映射一致性
  ...
  - **新增**：idempotency_cache 表提供幂等性保护支持
...
- 索引策略
  ...
  - **新增**：idempotency_cache 表需要针对 key 字段的唯一索引
...
- **idempotency_cache 表**：`CREATE UNIQUE INDEX idx_idempotency_key ON idempotency_cache(key);`
...
- 常见问题定位
  ...
  - **新增**：Alembic 迁移失败：检查幂等性缓存表和迁移脚本的执行状态
- 日志与审计
...
- **Alembic 迁移问题**：检查 alembic_version 表中的迁移版本状态，确认幂等性缓存是否正常工作
...
**Alembic 迁移系统排查**：
- 检查 env.py 中的幂等性保护配置是否正确
- 验证 script.py.mako 模板中的 MySQL 8.4 兼容性设置
- 确认 alembic.ini 配置文件中的数据库连接参数
- 检查迁移脚本的版本管理和回滚机制
...
- **份额精度标准化**：**所有份额相关字段统一标准化为精确的2位小数，确保财务计算的准确性和一致性**
- **Alembic 迁移增强**：**新增幂等性保护机制和 MySQL 8.4 兼容性支持，大幅提升迁移可靠性和执行效率**

建议在生产环境持续监控任务执行与错误日志，定期评估索引与快照生成策略，确保系统稳定高效运行。对于字段重命名变更，需要特别注意向后兼容性和数据迁移的一致性。**对于派生字段，建议建立数据完整性监控，确保新快照始终包含完整的派生字段值**。**对于份额精度标准化，建议建立数据质量监控，确保所有份额字段始终保持精确的2位小数**。**对于 Alembic 迁移系统，建议建立迁移执行监控和错误告警机制**。
...
- task_execution_log：任务执行日志
- nav_sync_detail：净值同步明细
- idempotency_cache：幂等性缓存（**新增，支持迁移幂等性保护**）
...
**幂等性保护机制**：
- 通过 idempotency_cache 表记录迁移执行状态
- 支持迁移脚本的重复执行而不产生副作用
- 提供事务级别的迁移操作保证

**MySQL 8.4 兼容性**：
- 优化 Alembic 迁移脚本以支持 MySQL 8.4 的新特性
- 调整数据类型和语法以符合新版本要求
- 改进连接池配置以适应高并发场景
