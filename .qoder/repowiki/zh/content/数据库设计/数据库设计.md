...
## 更新摘要
**变更内容**
- 新增在途资金模型（Issue #93），支持非对称状态管理跨天现金转移
- 扩展 product_code 字段长度至 String(20)，支持 IN_TRANSIT_BUY/IN_TRANSIT_SELL 虚拟产品
- portfolio_value_snapshot 表新增 in_transit_total 字段记录在途资金总额
- 完善平台间现金转移的非对称状态逻辑：转出方当日确认，转入方次交易日确认
- 强化数据一致性约束，确保交易配对和状态同步的原子性
...
**最新更新**：支持平台间现金转移功能，通过 `transfer_group` 字段关联配对交易；申购赎回记录按平台拆分追踪；持仓快照按平台维度生成；份额变动事件表字段规范化，`event_date` 重命名为更具语义的 `ex_date`；**新增 investor_holding 表派生字段支持，提供市值、成本和利润的直接查询能力**。**最新增强**：通过 Alembic 迁移实现数据规范化，包括 transfer_group 非空约束、份额精度标准化、字段重命名治理，以及统一的量化系统确保数据一致性。**最新特性**：**引入在途资金模型，支持非对称状态管理的跨天现金转移，确保 D 日 NAV 不因在途转移虚跌**。
...
- 产品与市场
  - product：产品信息、确认天数、数据源与分类（**已扩展 product_code 至 String(20)**）
...
- 组合持仓与交易
  ...
  - portfolio_value_snapshot：组合单位净值与总值快照（**新增 in_transit_total 字段**）
- 日志系统
...
- **在途资金模型**：**支持非对称状态管理的跨天现金转移，确保 D 日 NAV 不因在途转移虚跌**

章节来源
- [portfolio_value_snapshot.py:16](file://backend/app/models/portfolio_value_snapshot.py#L16)
- [trade.py:20](file://backend/app/models/trade.py#L20)
- [cash_transfer_service.py:5-6](file://backend/app/services/cash_transfer_service.py#L5-L6)
...
### 产品信息表 product
- 主键：(code, market)（复合主键）
- 关键字段：name、product_type（ETF/OEF/LOF/CASH/**IN_TRANSIT**）、asset_class_code、confirm_days、is_qdii、data_source、fallback_source、data_source_status、last_sync_at、sync_error、created_at、updated_at
- 业务要点：LOF 拆分为两条记录（CN_EXCHANGE 与 CN_OTC）；确认天数按市场与是否 QDII 自动计算；**新增 IN_TRANSIT 虚拟产品支持在途资金管理**
- 约束：外键指向 asset_classification；UNIQUE 复合键

**重要更新**：
- **product_code 字段扩展**：从 String(10) 扩展至 String(20)，支持 IN_TRANSIT_BUY / IN_TRANSIT_SELL 等长命名
- **虚拟产品支持**：新增 IN_TRANSIT 产品类型，用于在途资金跟踪

章节来源
- [02-数据库设计.md](file://Docs/02-数据库设计.md)
- [0006_product_code_extend_and_in_transit.py:9-10](file://backend/alembic/versions/0006_product_code_extend_and_in_transit.py#L9-L10)
...
- 支持跨天到账（cross_day=true）：**非对称状态保证**——转出方 sell 腿当日 confirmed（confirm_date = transfer_date），转入方 buy 腿 pending（confirm_date = next_trading_day），确保 D 日 NAV 不因在途转移虚跌

**在途资金模型（Issue #93）**：
- **非对称状态管理**：跨天现金转移采用不对称确认逻辑，转出方资金划出即确认，转入方待到账才确认
- **NAV 保护机制**：通过延迟确认转入方，避免在途期间组合净值虚跌
- **配对交易保证**：transfer_group 确保买卖两腿的原子性和一致性

章节来源
- [02-数据库设计.md](file://Docs/02-数据库设计.md)
- [trade.py:14](file://backend/app/models/trade.py#L14)
- [cash_transfer_service.py:112-117](file://backend/app/services/cash_transfer_service.py#L112-L117)
...
- 关键字段：total_value、**total_shares**、unit_price、unit_price_change_pct、**in_transit_total**、created_at
- 约束：外键关联 portfolio；UNIQUE 复合键

**重要更新**：
- **新增 in_transit_total 字段**：Numeric(15, 4)，server_default='0'，记录在途资金总额
- **精度调整**：`total_shares` 和 `frozen_shares` 字段精度从 Numeric(15,4) 调整为 Numeric(15,2)

**在途资金跟踪**：
- `in_transit_total` 字段专门用于跟踪跨天现金转移的在途金额
- 配合非对称状态模型，确保组合净值计算的准确性
- 支持在途资金的实时监控和报表统计
...
**在途资金模型性能考虑**：
- `in_transit_total` 字段支持高效的在途资金统计查询
- transfer_group 索引优化配对交易的查询性能
- 非对称状态管理减少不必要的状态同步开销
...
- **在途资金监控**：检查 portfolio_value_snapshot.in_transit_total 字段是否正确反映在途金额
- **非对称状态验证**：确认跨天现金转移的 sell 腿已确认而 buy 腿仍 pending
...
**在途资金模型排查**：
- 检查 IN_TRANSIT_BUY/IN_TRANSIT_SELL 虚拟产品是否正确初始化
- 验证 in_transit_total 字段在快照生成时的正确计算
- 确认非对称状态逻辑在不同场景下的正确性
...
- **在途资金模型**：**支持非对称状态管理的跨天现金转移，确保 D 日 NAV 不因在途转移虚跌，提升组合净值计算的准确性**

**建议在生产环境持续监控任务执行与错误日志，定期评估索引与快照生成策略，确保系统稳定高效运行。对于字段重命名变更，需要特别注意向后兼容性和数据迁移的一致性。对于精度调整，需要验证所有相关业务逻辑的兼容性。对于量化系统，需要确保所有份额写入路径都正确使用 quantize_shares 函数。对于在途资金模型，需要重点关注非对称状态的一致性和 NAV 保护的准确性**。
...
- product：产品代码、市场、名称、类型、确认天数、是否 QDII、数据源、最后同步时间、错误信息、创建/更新时间（**已扩展 product_code 至 String(20)，支持 IN_TRANSIT 虚拟产品**）
...
- portfolio_value_snapshot：组合单位净值与总值快照（**新增 in_transit_total 字段记录在途资金**）
...
**非对称状态管理**：
- **跨天现金转移**：转出方 sell 腿当日 confirmed（confirm_date = transfer_date），转入方 buy 腿 pending（confirm_date = next_trading_day）
- **当天现金转移**：两腿均立即 confirmed，confirm_date = transfer_date
- **NAV 保护机制**：通过延迟确认转入方，避免在途期间组合净值虚跌

**虚拟产品支持**：
- **IN_TRANSIT_BUY**：买入在途资金虚拟产品
- **IN_TRANSIT_SELL**：卖出在途资金虚拟产品
- **product_type='IN_TRANSIT'**：特殊产品类型标识
- **confirm_days=0**：虚拟产品不参与确认天数计算

**数据完整性保证**：
- **transfer_group 非空约束**：每笔交易必须属于一个业务组
- **配对交易原子性**：confirm/unconfirm/cancel 操作保证两腿状态同步
- **唯一约束**：`(transfer_group, product_code, trade_type)` 防止重复确认
...
- [cash_transfer_service.py:112-117](file://backend/app/services/cash_transfer_service.py#L112-L117)
- [0006_product_code_extend_and_in_transit.py:19-22](file://backend/alembic/versions/0006_product_code_extend_and_in_transit.py#L19-L22)
...
**迁移 0006 - 在途资金模型支持**：
- **product_code 字段扩展**：从 String(10) 扩展至 String(20)，支持 IN_TRANSIT_BUY / IN_TRANSIT_SELL 等长命名
- **新增 in_transit_total 列**：portfolio_value_snapshot 表新增 in_transit_total 字段，Numeric(15, 4)，server_default='0'
- **虚拟产品种子数据**：插入 IN_TRANSIT_BUY 和 IN_TRANSIT_SELL 两条种子产品记录
- **幂等设计**：支持 create_all 后直接执行，MySQL 使用 ON DUPLICATE KEY UPDATE，SQLite 使用 INSERT OR IGNORE
- **不可逆迁移**：一旦存在 IN_TRANSIT 数据，downgrade 操作被标记为不可逆
...
- [0006_product_code_extend_and_in_transit.py:55-110](file://backend/alembic/versions/0006_product_code_extend_and_in_transit.py#L55-L110)