...
## 简介
本文件面向数据库管理员与开发者，系统化梳理 InvestRing 的数据库设计，覆盖 22 张核心表（14 张核心业务表 + 7 张日志系统表 + 1 张幂等性缓存表），并结合文档与 ORM 映射文件，给出表结构、主外键、索引与约束设计原则，以及数据验证、业务规则、完整性保障、访问模式、缓存策略、性能优化、数据生命周期与归档、迁移路径与版本管理策略。
