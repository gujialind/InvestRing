...
## 更新摘要
**所做更改**
- 新增在途资金功能，支持REST端点管理投资组合的在途资金状态
- 交易schema新增--cash-confirm-date选项专门用于卖交易的确认日期处理
- CLI命令相应更新了新参数以支持在途资金管理
- 增强了交易确认流程，提供更灵活的确认日期控制
...
## 简介
本文件为 InvestRing 后端的完整 API 文档，覆盖认证系统、投资人管理、投资组合管理、产品管理、持仓管理、交易管理、快照系统、任务管理、数据源配置、市场数据、日志管理、通知管理、份额变动事件、交易日历、在途资金管理等模块。文档提供每个 API 的 HTTP 方法、URL 模式、请求/响应模式、认证方式与权限控制、错误码说明、请求与响应示例路径、版本管理与兼容性策略、常见使用场景与最佳实践。

**更新** 本次更新新增了完整的在途资金功能，支持REST端点管理投资组合的在途资金状态，同时增强了交易schema的--cash-confirm-date选项专门用于卖交易的确认日期处理，CLI命令也相应更新了新参数以支持这些新功能。
...
A --> T["在途资金路由<br/>routers/cash_transfers.py"]
A --> U["OpenAPI规范<br/>export_openapi.py / openapi.json"]
```
...
- [backend/app/routers/cash_transfers.py:1](file://backend/app/routers/cash_transfers.py#L1)
- [backend/export_openapi.py:1](file://backend/export_openapi.py#L1)
- [backend/openapi.json:1](file://backend/openapi.json#L1)

## 核心组件
- 应用入口与路由注册：在主应用中注册认证、投资人、组合、产品、平台、交易、持仓、订阅、快照、任务、数据源、市场数据、日志、通知、份额事件、交易日历、在途资金等路由，并启用 CORS。
...
- **在途资金管理**：新增完整的在途资金功能，支持REST端点管理投资组合的在途资金状态和交易确认日期处理。
...
## 架构总览
下图展示 API 路由与核心依赖的关系，体现认证、权限、安全与业务模块之间的交互，以及新增的OpenAPI规范管理流程、增强的错误处理机制、完整的通知系统功能、净值覆盖率验证功能和在途资金管理功能。
...
CashR["在途资金路由<br/>routers/cash_transfers.py"]
end
...
- 接口清单
  ...
  - **新增** GET /api/portfolios/{code}/aggregation
    ...
    - 示例路径：[组合聚合示例:278-320](file://backend/app/routers/portfolios.py#L278-320)
...
- 接口清单
  ...
  - POST /api/trades/{id}/confirm
    ...
    - **更新** 新增--cash-confirm-date选项：专门用于卖交易的确认日期处理，提供更灵活的确认日期控制
...
**更新** 新增PRICE_NAV_MISMATCH错误码：在处理场外基金交易确认时，如果客户端提供的价格与系统获取的基金净值不一致，将返回此错误码。这有助于确保交易价格的准确性和一致性。**特别重要的是，新增了--cash-confirm-date选项专门用于卖交易的确认日期处理，为交易确认流程提供了更灵活的日期控制能力。**
...
- 接口清单
  - GET /api/cash-transfers
    - 查询参数：portfolio_code、status、page、page_size
    - 响应体：分页对象（按创建时间倒序）
    - 功能：查询投资组合的在途资金记录，支持按状态筛选
    - 示例路径：[在途资金列表示例:1-100](file://backend/app/routers/cash_transfers.py#L1-L100)
  - POST /api/cash-transfers
    - 请求体：CashTransferCreate（包含 portfolio_code、transfer_type、amount、expected_date 等）
    - 响应体：CashTransferResponse（状态默认 pending）
    - 权限：管理员；交易日校验；金额校验
    - 功能：创建新的在途资金记录，支持买入和卖出两种类型
    - 示例路径：[创建在途资金示例:103-200](file://backend/app/routers/cash_transfers.py#L103-L200)
  - GET /api/cash-transfers/{id}
    - 响应体：CashTransferResponse
    - 功能：获取单个在途资金记录的详细信息
    - 示例路径：[获取在途资金示例:203-220](file://backend/app/routers/cash_transfers.py#L203-L220)
  - POST /api/cash-transfers/{id}/confirm
    - 查询参数：confirm_date（可选）、actual_amount（可选）
    - 响应体：确认结果与 CashTransferResponse
    - 权限：管理员；交易日校验；实际金额校验
    - 功能：确认在途资金交易，支持自定义确认日期和实际金额
    - 示例路径：[确认在途资金示例:223-300](file://backend/app/routers/cash_transfers.py#L223-L300)
  - POST /api/cash-transfers/{id}/cancel
    - 响应体：{"message": "..."}
    - 权限：管理员；仅 pending 状态可取消
    - 功能：取消待处理的在途资金记录
    - 示例路径：[取消在途资金示例:303-320](file://backend/app/routers/cash_transfers.py#L303-L320)
  - PUT /api/cash-transfers/{id}
    - 请求体：CashTransferUpdate
    - 响应体：CashTransferResponse
    - 功能：更新在途资金记录信息
    - 示例路径：[更新在途资金示例:323-340](file://backend/app/routers/cash_transfers.py#L323-L340)
  - DELETE /api/cash-transfers/{id}
    - 响应体：{"message": "..."}
    - 权限：管理员；仅 pending 状态可删除
    - 功能：删除在途资金记录
    - 示例路径：[删除在途资金示例:343-360](file://backend/app/routers/cash_transfers.py#L343-L360)

**更新** 新增完整的在途资金管理功能：该功能专门处理投资组合的在途资金状态，支持买入和卖出两种类型的资金转移。通过REST端点提供完整的CRUD操作，包括创建、查询、确认、取消和删除等操作。新增的--cash-confirm-date选项为卖交易提供了专门的确认日期处理能力，确保交易确认日期的准确性。

**章节来源**
- [backend/app/routers/cash_transfers.py:1-360](file://backend/app/routers/cash_transfers.py#L1-L360)
...
- 交易与风控
  ...
  - **新增**：使用--cash-confirm-date选项处理卖交易的确认日期，确保交易确认日期的准确性。
- 在途资金管理最佳实践
  - 创建在途资金记录时，应设置合理的预期到账日期。
  - 确认在途资金时，应验证实际金额与预期金额的差异是否在允许范围内。
  - 定期清理已过期的在途资金记录，保持数据整洁。
- 快照与数据一致性
...
- [backend/app/routers/cash_transfers.py:103-360](file://backend/app/routers/cash_transfers.py#L103-L360)
...
- **在途资金管理性能**：在途资金记录查询应支持分页和状态筛选，避免大量数据一次性加载。
...
- 业务校验失败
  ...
  - **新增** 在途资金确认失败：422，提示 InTransitConfirmError - 在途资金确认失败，检查确认日期和实际金额。
- 任务执行异常
...
- **在途资金管理问题**
  - 在途资金创建失败：检查投资组合状态、金额正负性和交易日有效性。
  - 在途资金确认失败：检查确认日期是否为交易日，验证实际金额与预期金额的差异。
  - 在途资金查询为空：检查投资组合代码和状态筛选条件是否正确。

**更新** 新增PRICE_NAV_MISMATCH错误码说明：当处理场外基金交易确认时，如果客户端提供的价格与系统获取的基金净值不一致，将返回此错误码。建议客户端在遇到此错误时，重新获取最新的基金净值或允许系统自动确定确认价格。**特别重要的是，新增了InTransitConfirmError错误码，用于处理在途资金确认过程中的各种异常情况，包括确认日期无效、金额不匹配等问题。**
...
sequenceDiagram
participant Client as "客户端"
participant Cash as "在途资金路由"
participant DB as "数据库"
participant Port as "投资组合"
participant Cal as "交易日历"
Client->>Cash : POST /api/cash-transfers
Cash->>Port : 验证投资组合状态
Port-->>Cash : 返回状态
Cash->>Cal : 校验交易日
Cal-->>Cash : 交易日
Cash->>DB : 创建在途资金记录
DB-->>Cash : 返回记录ID
Client->>Cash : POST /api/cash-transfers/{id}/confirm
Cash->>Cal : 验证确认日期
Cal-->>Cash : 确认日期有效
Cash->>DB : 更新在途资金状态
DB-->>Cash : 返回确认结果
