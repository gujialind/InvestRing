...
## 更新摘要
**所做更改**
- 新增了通知系统相关的API端点，包括通知查询、标记已读和批量操作功能
- 增强了现有API的功能描述，特别是交易确认流程中的错误处理机制
- 完善了错误码分类和说明文档，新增PRICE_NAV_MISMATCH错误代码
- 更新了OpenAPI规范管理流程，支持完整的API文档自动化生成
...
**更新** 新增了完整的通知系统API端点，支持通知查询、标记已读和批量操作功能。同时增强了交易确认流程的错误处理能力，新增PRICE_NAV_MISMATCH错误代码用于处理场外基金净值与提供价格不一致的业务场景。OpenAPI规范管理功能进一步完善，支持完整的API文档自动化生成和Apifox工具导入。
...
## 架构总览
下图展示 API 路由与核心依赖的关系，体现认证、权限、安全与业务模块之间的交互，以及新增的OpenAPI规范管理流程、增强的错误处理机制和完整的通知系统功能。
...
## 结论
本 API 文档覆盖 InvestRing 后端主要业务模块，明确了认证与权限、数据模型、关键流程与错误处理。新增的OpenAPI规范管理功能提供了完整的API文档自动化解决方案，支持Apifox等工具导入，便于团队协作和API文档维护。同时，新增的PRICE_NAV_MISMATCH错误码增强了场外基金交易的错误处理能力，提高了系统的健壮性和用户体验。新增的完整通知系统API端点进一步丰富了系统功能，支持管理员进行通知管理和用户通知服务。建议在生产环境中补充速率限制、审计日志与监控告警，并持续完善版本演进策略与向后兼容保障。
