...
## 更新摘要
**变更内容**
- 新增增量现金计算逻辑的测试覆盖，验证manual market value覆盖在快照生成过程中的正确应用
- 更新了calculate_available_cash函数的基线计算逻辑，从全量流水重算改为直接读快照表
- 完善了快照服务的测试策略，反映新的增量计算方法
- 增强了现金市值覆盖继承机制的文档说明
...
- **新增**：增量现金计算逻辑与manual market value覆盖继承机制
...
**新增**：增量现金计算采用快照基线+窗口增量模式，确保manual market value覆盖的正确继承。
...
### 增量现金计算逻辑（新增）
**更新**：calculate_available_cash函数实现了增量现金计算基线对齐快照口径的新机制。
...
- **新增**：增量现金计算优化：直接读取快照表基线，O(1)查询复杂度，避免全量流水重算
...
## 结论
快照系统通过严格的生成顺序、完善的依赖校验与冻结份额计算，确保了组合价值、资产配置与投资人份额在交易日维度上的准确与一致。**新增的增量现金计算逻辑进一步提升了系统的准确性与性能**，通过直接读取快照表基线避免了全量流水重算的性能开销，同时确保了manual market value覆盖值的正确继承。管理员可通过接口进行生成、重算与维护，普通用户可查询组合快照状态与收益统计。建议结合前端缓存与事务保障，在保证数据质量的同时提升性能与可用性。
...
- [backend/tests/unit/test_position_service.py:88-181](file://backend/tests/unit/test_position_service.py#L88-L181)
- [backend/tests/unit/test_snapshot_service.py:566-662](file://backend/tests/unit/test_snapshot_service.py#L566-L662)