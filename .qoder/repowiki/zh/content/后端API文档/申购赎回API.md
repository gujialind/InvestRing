...
- **重要更新** 赎回金额量化处理：订阅确认流程中赎回金额(shares × nav)现在使用quantize_amount量化到2位小数，订阅创建时用户输入金额也经过量化验证
...
- **新增** 已确认订阅事件的安全保护机制
- **重要更新** 金额精度统一处理：赎回金额和申购金额均使用量化函数确保2位小数精度
...
- `confirm_single_subscription()`：确认单笔申购/赎回
  ...
  - 首次申购确认后自动激活组合状态
  - **重要更新** 赎回金额量化：使用quantize_amount将shares × nav结果量化到2位小数
...
- 数据一致性
  ...
  - **重要更新** 金额精度统一：所有金额计算使用ROUND_HALF_UP四舍五入到2位小数
- **新增** 服务层分离
  - 业务逻辑从路由层解耦，提高代码复用性
  - 统一的异常处理机制
- **新增** 安全验证
...
服务->>组合 : "检查是否为首次申购"
组合-->>服务 : "是/否"
service->>量化 : "quantize_amount(shares × nav)"
量化-->>service : "返回量化后的金额"
service->>模型 : "查询申请日快照获取净值"
模型-->>service : "返回净值或抛出异常"
...
- 份额/金额计算
  ...
  - 赎回：**重要更新** 确认时按净值计算金额 = shares × nav，并使用quantize_amount量化到2位小数
  - **重要更新** 用户输入金额量化：申购创建时用户输入的金额会先经过quantize_amount量化验证
...
SubType --> |subscribe| CheckAmount["校验金额>0并量化"]
...
- **重要更新** 量化工具依赖
  - quantize_amount：金额量化到2位小数（ROUND_HALF_UP）
  - quantize_shares：份额量化到2位小数（ROUND_HALF_UP）
...
- **重要更新** 量化处理优化：quantize_amount使用Decimal运算，避免浮点数精度问题
...
- **重要更新** 金额精度统一处理：通过quantize_amount确保所有金额计算都量化到2位小数，避免了浮点数精度问题
...
### **重要更新** 金额量化处理说明
- **赎回金额量化**：在订阅确认流程中，赎回金额计算公式为 shares × nav，然后使用quantize_amount函数量化到2位小数
- **申购金额量化**：在订阅创建时，用户输入的金额会先经过quantize_amount函数量化验证
- **量化规则**：使用ROUND_HALF_UP四舍五入模式，第3位小数>=5进位，<5舍去
- **精度保证**：所有金额计算都使用Decimal类型，避免浮点数精度问题
- **业务影响**：确保与真实交易平台的2位小数口径一致，微小误差计入基金财产

**章节来源**
- [backend/app/services/subscription_service.py:125-129](file://backend/app/services/subscription_service.py#L125-L129)
- [backend/app/services/subscription_service.py:288-291](file://backend/app/services/subscription_service.py#L288-L291)
- [backend/app/utils/quantize.py:41-52](file://backend/app/utils/quantize.py#L41-L52)
- [backend/tests/integration/test_amount_precision.py:144-176](file://backend/tests/integration/test_amount_precision.py#L144-L176)