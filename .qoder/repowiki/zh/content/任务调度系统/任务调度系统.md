...
## 更新摘要
**变更内容**
- 新增高级任务运行器系统 `task_runner.py`，实现自动化后台任务执行
- 重构任务执行逻辑，将核心业务函数从路由层提取到服务层
- 增强任务执行跟踪和错误处理机制
- 支持净值同步后自动触发快照生成
- 提供 CLI 和 API 双通道任务管理接口
...
## 架构总览
系统采用"数据库驱动 + 高级任务运行器 + 手动触发 + 交易日历约束"的智能任务调度方案：
- 应用启动时创建表并初始化三个核心任务记录
- 通过高级任务运行器集中管理任务执行逻辑
- 支持 API 和 CLI 双通道任务触发
...
Runner->>Svc : "根据任务类型执行具体逻辑"
Svc->>Cal : "查询/校验交易日"
Cal-->>Svc : "返回是否为交易日"
Svc-->>Runner : "返回执行结果"
Runner->>Svc : "净值同步完成，检查交易日"
Svc->>Svc : "自动触发组合日快照生成"
...
### 高级任务运行器（Task Runner）
**新增** 高级任务运行器是任务调度系统的核心组件，提供了集中化的任务执行引擎：
...
## 结论
InvestRing 的任务调度系统通过引入**高级任务运行器**，实现了更加模块化和可维护的任务管理架构。系统以"数据库驱动 + 高级任务运行器 + 手动触发 + 交易日历约束"为核心，结合智能联动机制，实现了对系统初始化、数据同步与日志清理的统一管理。通过完善的日志与明细记录，系统具备良好的可观测性与可维护性。未来可在此基础上引入周期性调度器，结合 Cron 字段实现自动化执行。
...
- **系统初始化任务**
  ...
  - 触发：应用启动时自动执行
- **数据同步任务**
  - 净值同步（nav_sync）：按产品与市场类型同步价格数据，支持增量与去重，成功后自动触发组合日快照生成
