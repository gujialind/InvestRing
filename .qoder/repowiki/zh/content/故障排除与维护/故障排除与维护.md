...
- [backend/tests/integration/test_portfolios.py](file://backend/tests/integration/test_portfolios.py)
- [backend/tests/integration/test_positions.py](file://backend/tests/integration/test_positions.py)
- [backend/tests/integration/test_market_resolve.py](file://backend/tests/integration/test_market_resolve.py)
- [backend/tests/integration/test_snapshot_catchup.py](file://backend/tests/integration/test_snapshot_catchup.py)
- [backend/tests/integration/test_tasks.py](file://backend/tests/integration/test_tasks.py)
- [backend/tests/integration/test_trading_day.py](file://backend/tests/integration/test_trading_day.py)
</cite>

## 更新摘要
**变更内容**
- 新增测试覆盖范围章节，详细说明388个通过测试的集成测试覆盖范围
- 更新故障排除指南中的测试验证方法
- 增强系统稳定性与质量保证相关内容
- 添加测试驱动的问题定位与修复流程
...
**更新** 新增全面的测试覆盖说明，包括投资组合、持仓、市场解析、快照追赶操作、任务管理和交易日功能的集成测试，共388个通过测试，确保系统稳定性和可靠性。
...
- **新增** 测试框架：完整的集成测试套件覆盖核心业务功能，确保代码质量和系统稳定性。
...
## 架构总览
后端通过 Uvicorn 承载 FastAPI 服务，前端通过 Next.js 提供管理与展示界面。数据库层由 SQLAlchemy 引擎与会话管理，配合 Alembic 进行迁移。日志系统通过专用表与 API 支撑审计与排障。测试框架确保各组件的稳定性和正确性。
...
- **新增** 测试性能：集成测试模拟真实场景，确保系统在负载下的性能表现。

## 测试覆盖与质量保证

### 集成测试覆盖范围
InvestRing 现已建立全面的测试体系，包含388个通过的集成测试，覆盖以下核心功能模块：

#### 投资组合管理测试
- 投资组合创建、更新、删除操作的完整性测试
- 投资组合权限控制与数据隔离验证
- 投资组合状态转换与业务规则校验
...
```mermaid
flowchart TD
TestRun["运行测试套件"] --> UnitTests["单元测试<br/>unit/*"]
TestRun --> IntegrationTests["集成测试<br/>integration/*"]
UnitTests --> Results["测试结果"]
IntegrationTests --> Results
Results --> Report["测试报告生成"]
Report --> Coverage["覆盖率分析"]
Coverage --> QualityGate["质量门禁"]
QualityGate --> Pass{"通过率 >= 95%?"}
Pass --> |是| Deploy["允许部署"]
Pass --> |否| Fix["修复问题"]
Fix --> TestRun
```
...
### 测试驱动的故障排除
- **问题定位**：当出现功能异常时，首先运行相关测试用例快速定位问题范围
- **回归验证**：修复问题后运行完整测试套件确保无回归问题
- **性能基准**：通过集成测试建立性能基准，监控系统性能变化
- **数据一致性**：利用测试数据验证业务逻辑的正确性和数据一致性
...
- [Docs/07-日志系统设计.md:353-396](file://Docs/07-日志系统设计.md#L353-396)
...
Restore --> Verify["功能验证"]
Verify --> TestSuite["运行测试套件"]
TestSuite --> Success{"测试通过?"}
Success --> |是| Done["完成"]
...
- [Docs/07-日志系统设计.md:430-449](file://Docs/07-日志系统设计.md#L430-449)
...
## 结论
本维护文档提供了从日常运维到紧急响应的全流程指导。通过规范化的配置管理、完善的日志体系、定期的备份与演练，以及清晰的故障排查流程，可显著降低系统风险并缩短故障恢复时间。**新增的全面测试覆盖体系**为系统稳定性提供了有力保障，388个集成测试确保了核心功能的正确性和可靠性。建议将本文作为标准作业程序（SOP）纳入团队知识库，并持续根据实际运行情况进行迭代完善。
...
- 常用命令与路径
  ...
  - **新增** 运行测试：pytest backend/tests/
  - **新增** 运行集成测试：pytest backend/tests/integration/
- 关键接口
...
- **新增** 测试相关
  - 运行所有测试：pytest
  - 运行特定测试：pytest test_file.py::test_function
  - 生成测试报告：pytest --junitxml=report.xml
  - 查看测试覆盖率：pytest --cov=backend --cov-report=html