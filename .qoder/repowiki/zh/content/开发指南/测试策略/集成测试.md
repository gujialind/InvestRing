...
- 数据库层：定义连接池参数、SQLite 特定 PRAGMA 设置、Session 工厂与依赖注入。
...
- SQLite 特定设置：WAL 模式、busy_timeout、synchronous 等提升并发与可靠性。
...
- 清洗策略：测试结束后回滚事务或删除临时数据；对 SQLite 使用 WAL 模式提升一致性与恢复能力。
...
- 数据库连接与事务测试
  ...
  - SQLite 特性：验证 WAL 模式、busy_timeout、synchronous 参数生效。
