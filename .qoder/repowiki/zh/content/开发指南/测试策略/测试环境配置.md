...
## 更新摘要
**变更内容**
- 新增 MySQL 8.4 服务容器配置，支持 CI 环境中的数据库测试
- 实现 SQLite 和 MySQL 双模式验证，确保跨数据库兼容性
- 复制生产 RDS 设置到测试环境，提高测试与生产环境的一致性
...
**最新更新**：本次更新重点引入了 MySQL 8.4 服务容器配置，实现了 SQLite 和 MySQL 双模式验证，并复制生产 RDS 设置以确保测试环境与生产环境的一致性。同时解决了依赖兼容性问题，完善了测试基础设施。
...
- CI/CD 配置：GitHub Actions 工作流与 Docker Compose 配置
...
CI[".github/workflows/ci.yml<br/>MySQL 8.4容器配置"]
DC["docker-compose.yml<br/>多数据库支持"]
end
subgraph "测试套件"
UNIT["unit/*<br/>单元测试"]
...
- **数据库配置**：SQLite（默认）或 MySQL（CI环境），支持事务隔离和 WAL 模式，新增 MySQL 8.4 容器支持
- **测试数据工厂**：完整的工厂函数集，支持所有核心模型的创建
- **认证系统**：JWT Token 生成和管理，支持 admin 和 viewer 角色
- **基础数据**：资产分类、平台、产品、交易日历、用户等预置数据
- **CI/CD 集成**：GitHub Actions 工作流配置，包含 MySQL 8.4 服务容器启动

**更新**：新增了 MySQL 8.4 服务容器配置，实现了 SQLite 和 MySQL 双模式验证，并复制生产 RDS 设置以确保测试与生产环境的一致性。
...
CFG --> DB["测试数据库引擎<br/>SQLite/MySQL双模式"]
...
CI[".github/workflows/ci.yml<br/>MySQL 8.4容器"] --> DB
DC["docker-compose.yml<br/>多数据库支持"] --> DB
```
...
### MySQL 8.4 服务容器配置
**重大更新**：在 CI 环境中新增了 MySQL 8.4 服务容器配置，支持双模式数据库验证

- **CI 配置**：在 `.github/workflows/ci.yml` 中添加了 MySQL 8.4 服务容器定义
- **Docker Compose 配置**：更新了 `docker-compose.yml` 以支持 MySQL 8.4 容器
- **双模式验证**：测试环境现在支持 SQLite 和 MySQL 两种数据库模式的验证
- **生产 RDS 设置复制**：测试环境的数据库配置复制了生产 RDS 的设置，确保环境一致性
...
- **双模式支持**：新增对 SQLite 和 MySQL 的双模式验证支持

**更新**：conftest.py 中增强了数据库客户端的异步上下文管理器优化，改进了 SQLite WAL 模式和事务隔离机制，并添加了 MySQL 8.4 容器的支持。

```mermaid
flowchart TD
Start(["进入测试"]) --> LoadCfg["加载配置<br/>读取 .env"]
LoadCfg --> BuildURL["构建 database_url"]
BuildURL --> CheckMode{"数据库模式检测"}
CheckMode --> |SQLite| SQLiteMode["SQLite模式<br/>WAL+事务隔离"]
CheckMode --> |MySQL| MySQLMode["MySQL 8.4模式<br/>InnoDB引擎"]
SQLiteMode --> CreateEngine["创建引擎/会话"]
MySQLMode --> CreateEngine
...
- **测试专用容器**：
  ...
  - **更新**：现在支持 MySQL 8.4 容器，与生产 RDS 设置保持一致
- **运行命令示例**：
...
- **双模式支持**：支持 SQLite 和 MySQL 两种模式的隔离测试

**更新**：conftest.py 实现了更完善的测试隔离机制，包括 SQLite WAL 模式配置和嵌套事务支持，并添加了 MySQL 8.4 容器的隔离配置。
...
- [backend/tests/integration/test_share_events.py:26-117](file://backend/tests/integration/test_share_events.py#L26-117)
...
CONF --> DB["数据库引擎<br/>SQLite/MySQL双模式"]
DB --> TESTS["测试套件"]
...
- **MySQL 优化**：MySQL 8.4 容器配置优化了 InnoDB 引擎参数，提升测试性能

**更新**：conftest.py 中针对 SQLite 启用了 WAL 模式和嵌套事务支持，显著提升了测试性能和稳定性。新增的 MySQL 8.4 容器配置也包含了性能优化参数。

## 故障排除指南
- **数据库连接失败**
  - 检查 .env.test 中的数据库参数与可达性
  - 确认测试数据库已创建且用户具备权限
  - **更新**：如果使用 MySQL 8.4 容器，检查容器状态和网络配置
- **初始化失败**
...
## 结论
通过统一的配置与数据库连接、完善的测试数据初始化、前后端分离的测试脚本与环境隔离策略，InvestRing 的测试环境能够稳定高效地支持本地开发与 CI/CD 流水线。本次重构引入了 MySQL 8.4 服务容器配置，实现了 SQLite 和 MySQL 双模式验证，并复制生产 RDS 设置以确保测试与生产环境的一致性。同时解决了关键的依赖兼容性问题，完善了测试数据工厂，并修复了多个测试用例的数据完整性问题。建议在团队内推广 .env.test 与独立测试数据库实践，并持续完善外部服务 Mock 与报告生成流程。

**更新总结**：本次测试基础设施重构显著提升了测试环境的稳定性和可靠性，特别是通过 MySQL 8.4 容器配置和生产 RDS 设置的复制，确保了测试环境与生产环境的高度一致性。

## 附录
- **快速检查清单**
  - 准备 .env.test 并设置测试数据库参数
  - 启动测试数据库容器并创建库（支持 SQLite 和 MySQL 8.4）
  - 执行 reset_db.py 与初始化脚本
  - 运行 pytest 与 Playwright 测试
  - 生成并归档测试报告
  - 验证 httpx 版本兼容性（>=0.26.0,<0.28.0）
  - 检查测试数据中的 platform_code 字段完整性
  - **更新**：验证 MySQL 8.4 容器配置和连接
...
- **MySQL 8.4 容器配置**
  ...
  - 数据持久化：挂载卷到 /var/lib/mysql

**更新**：新增了 MySQL 8.4 容器配置和相关的测试执行命令。