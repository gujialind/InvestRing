...
## 更新摘要
**变更内容**
- **简化**：本地开发环境配置大幅简化，移除了复杂的 Docker Compose 多服务配置
- **新增**：生产部署通过新的 ACR（Azure Container Registry）和 SSH 管道处理
- **优化**：专注于核心本地开发服务的简化配置
- **保持**：原有的依赖管理、基础容器化功能不变
...
## 简介
本指南面向 InvestRing 项目的开发者，提供简化后的完整环境搭建与运行说明。内容覆盖：
...
- API 密钥（Tushare）配置
- 一键启动脚本与本地开发服务器启动
- **简化**：简化的本地开发环境配置，移除复杂的多服务 Docker Compose 配置
- **新增**：生产部署通过 ACR 和 SSH 管道处理
- 常见问题排查与解决方案
...
- **简化**：移除了复杂的 Docker Compose 多服务配置，专注于核心本地开发服务
...
- 数据库初始化与迁移：提供检查与迁移脚本
- **简化**：本地开发环境配置大幅简化，移除复杂的多服务 Docker Compose 配置
- **新增**：生产部署通过 ACR 和 SSH 管道自动化处理
...
### 简化的本地开发环境配置
- **简化**：移除了复杂的 Docker Compose 多服务配置
- **聚焦**：专注于核心本地开发服务的简化配置
- **优势**：降低本地开发环境复杂度，提升开发体验
- **替代方案**：生产部署通过 ACR 和 SSH 管道处理
...
## 结论
通过本指南，您可以完成 InvestRing 项目的简化版本地开发环境搭建与运行。项目已大幅简化本地开发环境配置，移除了复杂的 Docker Compose 多服务配置，专注于核心本地开发服务。生产部署通过 ACR 和 SSH 管道自动化处理，提供更好的可维护性和部署效率。建议在开发过程中：
- 使用一键启动脚本快速验证环境
- 正确配置 .env 与数据库连接
- 合理使用迁移脚本管理数据库演进
- 前后端联调时关注代理与路由行为
- **简化**：利用简化的本地开发环境配置，享受更流畅的开发体验
- **新增**：生产部署通过 ACR 和 SSH 管道自动化处理
