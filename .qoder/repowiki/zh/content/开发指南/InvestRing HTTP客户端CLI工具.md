...
## 简介
InvestRing HTTP客户端CLI是一个轻量级命令行工具，通过REST API与后端服务通信，实现投资组合、产品、平台、交易、申赎、份额事件、市场数据、任务与快照等全量管理能力。该工具仅依赖typer与httpx两个库，可在任意设备上独立安装使用，无需后端源码或数据库访问权限。
...
```mermaid
graph TB
A["ir-cli/pyproject.toml<br/>定义包名、脚本入口"] --> B["ir_cli/main.py<br/>注册14个命令组"]
...
- [client.py:1-163](file://ir-cli/ir_cli/client.py#L1-L163)
...
- 命令组（commands/*）
  - auth、investor、portfolio、position、sub、trade、share-event、market、product、platform、system、log、task、snapshot共14个组，覆盖CRUD与业务流程操作
...
participant CMD as "命令组(如auth/login)"
participant C as "APIClient"
participant S as "后端服务"
U->>CLI : 执行 ir auth login --code ... --password ...
CLI->>CMD : 路由到对应命令函数
CMD->>C : post("/api/auth/login", body)
C->>S : HTTP POST /api/auth/login
S-->>C : 200 {token, expires_at, user}
C-->>CMD : {"data" : {...}}
CMD->>CMD : 保存token到 ~/.ir/token.json
CMD-->>U : 成功输出 JSON
```
...
- 响应处理：
  - 401/403/404/422/5xx分别映射为AUTH_REQUIRED/FORBIDDEN/NOT_FOUND/VALIDATION_ERROR/SERVER_ERROR
...
图表来源
- [client.py:38-74](file://ir-cli/ir_cli/client.py#L38-L74)
- [client.py:98-106](file://ir-cli/ir_cli/client.py#L98-L106)
- [client.py:138-158](file://ir-cli/ir_cli/client.py#L138-L158)

章节来源
- [client.py:14-36](file://ir-cli/ir_cli/client.py#L14-L36)
- [client.py:38-96](file://ir-cli/ir_cli/client.py#L38-L96)
- [client.py:98-163](file://ir-cli/ir_cli/client.py#L98-L163)
...
- [client.py:108-116](file://ir-cli/ir_cli/client.py#L108-L116)
- [config.py:40-48](file://ir-cli/ir_cli/config.py#L40-L48)
...
participant S as "后端服务"
U->>CLI : 指定产品代码与市场类型
CLI->>C : POST /api/market-data/products/{code}/{market}/sync-history
C->>S : 触发后端同步完整历史
S-->>C : 返回同步结果
C-->>CLI : {"data" : ...}
CLI-->>U : 输出成功JSON
...
章节来源
- [client.py:38-96](file://ir-cli/ir_cli/client.py#L38-L96)
- [client.py:98-136](file://ir-cli/ir_cli/client.py#L98-L136)
- [auth.py:52-64](file://ir-cli/ir_cli/commands/auth.py#L52-L64)

## 结论
InvestRing HTTP客户端CLI以最小依赖实现了完整的投资管理系统命令行能力，具备良好的可移植性与易用性。通过统一的认证、配置、错误处理与输出协议，CLI能够稳定地与后端协作，满足日常运维与自动化场景需求。
