...
## 更新摘要
**变更内容**
- **金额精度统一**：现金分红计算（entitlement_shares × div_cash）和强制调整现金变更现在应用金额量化，确保所有份额变动事件的货币金额遵循相同的2位小数精度规则
- **增强计算逻辑**：在 `_compute_event_fields` 函数中统一应用 `quantize_amount` 进行金额量化处理
- **测试覆盖完善**：新增针对现金分红和强制调整的金额精度测试用例
...
## 简介
本文件系统化阐述"份额变动事件"功能的增强设计与实现，重点介绍新增的父子事件关系管理系统、平台级与基金级事件分类机制、严格的日期验证规则和平台覆盖率检查功能。该功能现已升级为完整的现代化系统，支持所有CRUD操作，包括创建、查询、确认、取消、更新、删除和取消确认，并提供直观的可视化操作体验。**最新增强包括统一的金额精度处理，确保所有现金相关计算的精确性和一致性。**
...
Router --> Service["份额变动事件服务<br/>share_change_event_service.py"]
Service --> Quantize["金额量化工具<br/>utils/quantize.py"]
Service --> Model["ORM模型<br/>models/share_change_event.py"]
...
- **统一的金额量化处理**：通过 `quantize_amount` 工具确保所有现金相关计算的2位小数精度，包括现金分红和强制调整。
...
## 架构概览
份额变动事件处理遵循"公告期-权益登记日-执行日"的三阶段流程，结合交易日历与持仓快照进行严格校验，确保净值与会计平衡。新的前端界面提供了完整的用户交互体验，支持实时状态更新和错误处理，同时支持父子事件的自动拆分和管理。**最新的金额量化机制确保所有现金相关计算的一致性和精确性。**
...
participant S as "服务层<br/>share_change_event_service.py"
...
R->>S : create_share_change_event()
S->>Q : quantize_amount() for cash_change
S->>DB : 插入 share_change_event
DB-->>S : 提交成功
S-->>R : 返回事件
R-->>API : 返回事件
API-->>FE : 显示创建成功
...
API->>R : 校验状态为 pending
R->>S : confirm_share_change_event()
S->>S : _compute_event_fields()
S->>Q : quantize_amount() for dividend calculation
S->>DB : 更新事件为 confirmed
DB-->>S : 提交成功
S-->>R : 返回确认结果
R-->>API : 返回确认结果
API-->>FE : 显示确认成功
...
- [backend/app/services/share_change_event_service.py:306-342](file://backend/app/services/share_change_event_service.py#L306-L342)
...
### 事件类型与业务规则
- **事件类型**
  - 现金分红：份额不变，现金增加 = entitlement_shares × div_cash（量化到2位小数）
  - 分红再投资：份额增加，现金不变
  - 份额拆分：按比例扩大份额
  - 份额合并：按比例缩小份额
  - 红股（送股）：按比例增加份额
  - 强制调整：管理员手工指定份额与现金变化（均量化到2位小数）
- **事件分级模型**
...
### 会计处理与净值影响
- **现金分红**
  - 份额不变，单位净值除权；现金增加 = 权益登记日份额 × 每份分红金额（**量化到2位小数**）
- **分红再投资**
...
### 事件影响计算与数据一致性
- **现金分红**：cash_change = entitlement_shares × div_cash（**量化到2位小数**）；基金份额不变，单位净值除权；现金增加
...
- **强制调整**：按管理员指定的 shares_change 与 cash_change 执行（**均量化到2位小数**）
...
### 金额量化处理机制
- **统一精度标准**：所有金额字段统一量化到2位小数，采用ROUND_HALF_UP舍入模式
- **现金分红计算**：`cash_change = quantize_amount(entitlement_shares × div_cash)`，确保计算结果的精确性
- **强制调整处理**：用户输入的 `shares_change` 和 `cash_change` 均经过量化处理
- **量化工具**：通过 `quantize_amount` 函数实现统一的金额量化逻辑
- **测试验证**：通过集成测试确保现金分红和强制调整的金额精度符合预期

**章节来源**
- [backend/app/services/share_change_event_service.py:42-69](file://backend/app/services/share_change_event_service.py#L42-L69)
- [backend/app/utils/quantize.py:41-52](file://backend/app/utils/quantize.py#L41-L52)
- [backend/tests/integration/test_amount_precision.py:181-201](file://backend/tests/integration/test_amount_precision.py#L181-L201)
...
R --> S["服务层<br/>share_change_event_service.py"]
S --> Q["量化工具<br/>quantize.py"]
M --> DB["数据库表"]
P --> DB
T --> DB
PR --> DB
...
- **金额精度问题**
  - 现象：现金分红或强制调整的金额计算不符合预期
  - 处理：检查输入数据的精度，确认量化逻辑正确执行
- **前端错误提示**
...
## 结论
份额变动事件功能通过增强的父子事件关系管理系统、完整的CRUD操作支持、严格的交易日与持仓快照校验、清晰的事件类型与会计处理规则、完善的前后端接口与审计追踪，实现了对份额变动的准确记录与净值一致性保障。新的React界面提供了直观的用户体验，配合服务层的快照检查和现代化的可视化交互，能够满足运营与合规需求，显著提升了系统的易用性和维护性。**最新的金额量化机制进一步增强了系统的精确性和可靠性，确保所有现金相关计算的一致性。**父子事件关系和平台覆盖率检查机制进一步增强了系统的灵活性和准确性。最新的增强包括默认阻断的平台覆盖率检查、完整的unconfirm功能和详细的级联回退日志，使系统在数据一致性和可追溯性方面更加完善。**特别是现金分红计算和强制调整现金变更的金额量化处理，确保了财务数据的精确性和合规性。**

## 附录
- **事件类型与计算公式速查**
  - 现金分红：份额不变，现金增加 = quantize_amount(entitlement_shares × div_cash)
  - 分红再投资：份额增加 = entitlement_shares × div_cash ÷ reinvest_nav，现金不变
  - 份额拆分：shares_after = quantize_shares(entitlement_shares × ratio)，单位净值同步调整
  - 份额合并：shares_after = quantize_shares(entitlement_shares ÷ ratio)，单位净值同步调整
  - 红股送股：shares_change = quantize_shares(entitlement_shares × ratio)
  - 强制调整：按管理员指定的 quantize_shares(shares_change) 与 quantize_amount(cash_change) 执行
- **父子事件关系速查**
...
- **金额精度规范**
  - 所有金额字段统一量化到2位小数
  - 使用ROUND_HALF_UP舍入模式
  - 现金分红计算：entitlement_shares × div_cash → quantize_amount()
  - 强制调整：用户输入的shares_change和cash_change均量化处理
