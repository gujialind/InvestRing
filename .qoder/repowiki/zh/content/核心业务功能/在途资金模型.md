...
- [test_cli_service_parity.py](file://backend/tests/integration/test_cli_service_parity.py)
- [test_issue_88_90_91.py](file://backend/tests/integration/test_issue_88_90_91.py)
</cite>

## 更新摘要
**已完成的更改**
- 更新了非对称状态模型的测试断言，反映跨天现金转移的 sell/buy 腿不同确认状态
- 修正了基金买入配对 CASH 腿的确认日期逻辑，确保 T 日扣款与基金确认日分离
- 增强了数据库迁移 0006 的可空约束处理，支持 product.code 字段扩展
- 完善了集成测试对非对称模型行为的覆盖验证
...
- 集成测试：test_in_transit、test_cli_service_parity、test_issue_88_90_91 覆盖在途生命周期与非对称模型行为
...
- [cash_transfer_service.py:1-213](file://backend/app/services/cash_transfer_service.py#L1-L213)
...
- [cash_transfer_service.py:30-132](file://backend/app/services/cash_transfer_service.py#L30-L132)
...
**章节来源**
- [position_service.py:1-200](file://backend/app/services/position_service.py#L1-L200)
- [position_service.py:200-399](file://backend/app/services/position_service.py#L200-L399)
...
- **更新**：添加了对 product.code 字段的 NOT NULL 约束处理，确保 MySQL 主键列的完整性。

**章节来源**
- [0006_product_code_extend_and_in_transit.py:1-124](file://backend/alembic/versions/0006_product_code_extend_and_in_transit.py#L1-L124)

### 非对称状态模型测试覆盖
- **TestCashTransferAsymmetricParity**：验证跨天现金转移的非对称状态，sell 腿当日 confirmed，buy 腿 pending 至下一交易日。
- **TestCrossPlatformCashLeg**：验证基金买入配对 CASH 腿的确认日期逻辑，确保 T 日扣款与基金确认日分离。
- 测试断言已更新以适配 #93 非对称模型语义，确保 sell/buy 两腿的不同确认状态得到正确验证。

**章节来源**
- [test_cli_service_parity.py:210-290](file://backend/tests/integration/test_cli_service_parity.py#L210-L290)
- [test_issue_88_90_91.py:287-415](file://backend/tests/integration/test_issue_88_90_91.py#L287-L415)
...
- [cash_transfer_service.py:135-169](file://backend/app/services/cash_transfer_service.py#L135-L169)
...
- **更新**：增强的测试覆盖确保非对称模型行为的正确性，特别是跨天现金转移中 sell/buy 两腿的不同确认状态。
