...
## 更新摘要
**变更内容**
- 份额计算精度改进：申购管理中的份额计算采用新的 ROUND_HALF_UP 四舍五入规则，提高基金申购计算的准确性
- 量化工具增强：优化数值处理逻辑，确保所有份额计算遵循统一的四舍五入标准
- 数据一致性提升：通过标准化的精度处理，消除因浮点数运算导致的精度差异
- 前端显示优化：份额相关界面元素适配新的精度要求，提供更准确的数值展示
...
**更新** 本次更新重点改进了申购管理中的份额计算精度，采用 ROUND_HALF_UP 四舍五入规则，显著提高了基金申购计算的准确性和一致性。
...
## 核心组件
- 订阅模型（Subscription）：持久化存储申购/赎回申请的关键字段，包括组合代码、投资人代码、申请类型、金额/份额、确认净值、申请日期、确认日期、状态、备注等。**更新** 份额字段已标准化为2位小数精度，并采用 ROUND_HALF_UP 四舍五入规则。
- 订阅路由（Router）：提供查询、创建、详情、确认、取消、更新、删除等接口；内置交易日校验、状态流转、可用份额校验等业务逻辑。**更新** 所有份额计算现在使用 ROUND_HALF_UP 四舍五入规则进行标准化处理。
- 订阅Schema：定义创建、更新、响应的数据结构，确保前后端数据契约一致。**更新** 份额字段验证规则已调整为支持2位小数和精确的四舍五入处理。
- 权限依赖（get_current_user/get_current_admin）：基于JWT Token进行身份认证与授权，区分viewer/admin权限。
- 交易日历（TradingCalendar）：用于判断交易日与T+1确认日期推导。
- 快照服务（SnapshotService）：在生成/重算快照时，纳入已确认的申购/赎回对份额与成本的影响。**更新** 快照计算现在遵循统一的份额精度标准和四舍五入规则。
- **新增** 量化工具（Quantize）：提供数值精度处理函数，确保份额计算的准确性，采用标准的 ROUND_HALF_UP 四舍五入算法。
- **新增** 交易工具（TradingUtils）：封装交易相关的通用计算逻辑，包含份额精度处理和四舍五入规则。
- **新增** 数据迁移脚本：确保历史数据的份额精度一致性，应用新的四舍五入规则。
- 前端页面与Hooks：提供订阅列表、创建、确认、取消等交互与状态管理。**更新** 前端显示逻辑已适配新的份额精度要求和四舍五入规则。

**更新** 新增了量化工具和交易工具模块，增强了份额精度处理的准确性和一致性，特别是采用了标准的 ROUND_HALF_UP 四舍五入规则。
...
## 架构总览
订阅管理贯穿"前端交互 → 后端路由 → 数据模型/服务 → 数据库"的完整链路。关键流程包括：提交申请（校验交易日、组合状态、投资人存在性）、可用份额校验（针对赎回）、确认（计算份额/金额、更新净值、首笔确认激活组合）、取消（仅待确认状态）、查询与权限控制。**更新** 所有份额计算现在通过量化工具进行标准化处理，采用 ROUND_HALF_UP 四舍五入规则，确保精度一致性。
...
Router->>Utils : 标准化份额精度 (ROUND_HALF_UP)
...
Router->>Utils : 计算标准化份额 (ROUND_HALF_UP)
...
### 订阅模型与数据结构
- 字段要点：组合代码、投资人代码、申请类型（subscribe/redeem）、金额/份额、确认净值、申请/确认日期、状态（pending/confirmed/cancelled）、备注、时间戳。**更新** 份额字段已标准化为2位小数精度，并采用 ROUND_HALF_UP 四舍五入规则。
- 关系：外键关联组合与投资人；与交易日历配合推导确认日期；与快照服务共同决定份额与成本变化。
- **新增** 数据迁移：通过Alembic迁移脚本确保历史数据的份额精度一致性，应用新的四舍五入规则。
...
+string sub_type
+decimal amount
+decimal shares (2位小数, ROUND_HALF_UP)
+decimal unit_price
+date apply_date
+date confirm_date
+string status
...
- 申购（subscribe）
  ...
  - 确认：根据确认净值计算份额；首笔确认时若组合为草稿则激活。**更新** 份额计算现在使用 ROUND_HALF_UP 四舍五入规则的标准化2位小数精度。
- 赎回（redeem）
  - 输入：组合代码、投资人代码、申请份额、申请日期、备注。
  - 可用份额校验：扣除待确认/已确认但未出快照的赎回份额；确保不超过最新快照份额。**更新** 份额比较现在基于标准化精度和四舍五入规则。
  - 确认：根据申请日净值计算金额。
- 参数约束
  - 金额/份额必须大于0；
  - 申请日期必须为交易日；
...
**更新** 所有份额相关的参数验证和计算都采用了标准化的 ROUND_HALF_UP 四舍五入规则，确保数据一致性和计算准确性。
...
- 生命周期关键节点
  - 创建：写入待确认记录。
  - 确认：计算份额/金额、更新净值、更新确认日期；首笔确认激活组合。**更新** 份额计算现在遵循 ROUND_HALF_UP 四舍五入规则和标准化精度。
  - 取消：仅待确认可取消。
...
已确认 --> [*]
已取消 --> [*]
note right of 待确认 : ROUND_HALF_UP四舍五入\n确保数据一致性
note right of 已确认 : ROUND_HALF_UP四舍五入\n防止精度差异
```
...
- 创建订阅
  - 仅管理员可创建；校验交易日、组合状态、投资人存在性、金额/份额合法性。**更新** 份额验证现在包含精度检查和四舍五入规则验证。
- 获取订阅详情
  - viewer仅可查看自身记录。
- 确认订阅
  - 仅管理员；校验状态为待确认；自动推导确认日期；计算份额/金额；首笔确认激活组合。**更新** 份额计算使用 ROUND_HALF_UP 四舍五入规则和标准化精度。
- 取消订阅
  - 仅管理员；仅待确认可取消。
- 更新/删除
  - 仅管理员；支持更新部分字段与物理删除。
...
- 确认时净值与份额/金额计算
  - 申购：金额 ÷ 确认净值 = 份额；首笔默认净值为1.0000。**更新** 份额计算现在使用 ROUND_HALF_UP 四舍五入规则的标准化2位小数精度。
  - 赎回：份额 × 确认净值 = 金额。**更新** 份额处理现在遵循统一精度标准和四舍五入规则。
- 可用份额与可用现金计算
  - 赎回前实时计算可用份额：最新快照份额 − 待确认赎回 − 已确认但未出快照的赎回。**更新** 份额比较现在基于标准化精度和四舍五入规则。
  - 可用现金计算：纳入已确认但未出快照的申购/赎回与待确认买入/卖出影响。

**更新** 所有份额相关的计算和比较操作现在都使用 ROUND_HALF_UP 四舍五入规则和标准化的精度处理，确保数据一致性和计算准确性。
...
- 类型定义
  - 前后端共享Subscription/Create/Update类型，保证字段一致性。**更新** 份额字段类型已调整为支持2位小数和四舍五入规则。

**更新** 前端显示逻辑已适配新的份额精度要求和四舍五入规则，所有份额相关界面元素现在显示2位小数，并提供更准确的数值展示。
...
### 份额精度标准化机制
**更新** 份额精度标准化是本次更新的核心改进之一，采用 ROUND_HALF_UP 四舍五入规则，确保所有份额计算和显示的精确度统一。

- 精度标准
  - 所有份额字段统一为2位小数精度。
  - 计算过程中使用 ROUND_HALF_UP 四舍五入规则进行精度处理。
  - 数据库层面通过迁移脚本确保历史数据的一致性。
- 量化工具实现
  - 提供round_to_2_decimals函数进行数值四舍五入到2位小数。
  - 提供standardize_precision函数进行批量精度处理。
  - 集成到所有份额相关的计算逻辑中，确保四舍五入规则的一致性。
- 数据迁移
  - 通过Alembic迁移脚本更新数据库schema。
  - 转换历史数据以符合新的精度要求和四舍五入规则。
  - 确保数据完整性约束。
...
- 组件耦合
  ...
  - **新增** 依赖量化工具进行份额精度处理，采用 ROUND_HALF_UP 四舍五入规则。
  - **新增** 依赖交易工具进行通用交易逻辑处理。
...
**更新** 新增了量化工具和交易工具的依赖关系，增强了份额精度处理能力，特别是采用了标准的 ROUND_HALF_UP 四舍五入规则。
...
SubRouter --> Quantize["量化工具 (ROUND_HALF_UP)"]
...
- **新增** 份额精度处理性能
  - 量化工具采用高效的 ROUND_HALF_UP 四舍五入计算方法。
  - 批量精度处理避免重复计算。
  - 数据库层面的精度约束减少运行时处理开销。
...
- 常见错误与定位
  ...
  - 金额/份额非法：申购金额与赎回份额必须大于0；路由会返回422。**更新** 份额精度验证现在检查2位小数格式和四舍五入规则。
...
- **新增** 份额精度相关问题
  - 份额显示异常：检查量化工具是否正确应用 ROUND_HALF_UP 四舍五入规则。
  - 计算结果不一致：验证所有份额计算是否使用标准化精度和四舍五入规则。
  - 历史数据问题：确认数据迁移脚本是否正确执行，应用新的精度标准。

**更新** 新增了份额精度相关的故障排查指南，帮助快速定位和处理精度相关问题，特别是 ROUND_HALF_UP 四舍五入规则的应用问题。

章节来源
- [backend/app/routers/subscriptions.py:115-375](file://backend/app/routers/subscriptions.py#L115-375)
- [backend/app/dependencies.py:49-146](file://backend/app/dependencies.py#L49-L146)
- [frontend/src/hooks/useTrade.ts:234-324](file://frontend/src/hooks/useTrade.ts#L234-L324)
- [backend/app/utils/quantize.py](file://backend/app/utils/quantize.py)
- [ir-cli/ir_cli/commands/subscriptions.py](file://ir-cli/ir_cli/commands/subscriptions.py)
...
## 结论
订阅管理以"申购/赎回"为核心，围绕交易日、净值、份额与资金形成闭环。后端通过严格的参数校验与状态机保障业务正确性，前端提供直观的操作与反馈。**本次更新显著提升了系统的数值精度一致性和用户体验，特别是通过引入 ROUND_HALF_UP 四舍五入规则**。

**更新** 本次更新的主要改进包括：
- 引入 ROUND_HALF_UP 四舍五入规则，统一份额计算精度
- 增强量化工具模块，提供标准的数值处理能力
- 完善数据迁移脚本，确保历史数据的一致性和精度
- 优化前端显示逻辑，适配新的精度要求和四舍五入规则
...
- POST /subscriptions：创建订阅（管理员；校验交易日/组合/投资人/可用份额）**更新** 份额验证现在包含精度检查和四舍五入规则验证
- GET /subscriptions/{id}：获取订阅详情（viewer仅可查看自身）
- POST /subscriptions/{id}/confirm：确认订阅（管理员；校验状态；计算份额/金额；首笔确认激活组合）**更新** 份额计算使用 ROUND_HALF_UP 四舍五入规则和标准化精度
- POST /subscriptions/{id}/cancel：取消订阅（管理员；仅待确认可取消）
- PUT /subscriptions/{id}：更新订阅（管理员）
- DELETE /subscriptions/{id}：删除订阅（管理员）

**更新** 所有API的份额相关操作现在都使用 ROUND_HALF_UP 四舍五入规则和标准化的精度处理。
...
### 份额精度标准化规范
**更新** 份额精度标准化规范定义了系统中所有份额相关操作的精度要求，采用 ROUND_HALF_UP 四舍五入规则。

- 精度标准
  - 所有份额字段统一为2位小数精度。
  - 计算过程中使用 ROUND_HALF_UP 四舍五入到2位小数。
  - 显示时固定显示2位小数。
- 适用范围
  - 申购确认份额计算
  - 赎回申请份额处理
