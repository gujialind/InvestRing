...
- 份额变动事件（ShareChangeEvent）：除权除息、配股、分红等事件
...
- 投资人份额快照（InvestorHolding）
  ...
  - 冻结份额：来自待确认赎回
- 现金与份额可用性实时计算
  - 可用现金 = 最新快照现金 + 未生成快照期内确认申购 - 未生成快照期内确认赎回 - 待确认买入 - 未生成快照期内确认买入 + 未生成快照期内确认卖出
...
- 唯一索引：持仓/净值/份额快照按（组合, 产品, 市场, 日期）唯一
- 冻结控制：pending 交易导致的冻结份额/金额
- QDII 净值规则：必须使用 T 日净值，不得向前查找
- 可用性校验：下单前实时计算可用现金/份额，避免超买/超卖
- 依赖校验：生成快照前必须满足交易日、无待确认交易、价格数据完整、份额变动事件已确认
