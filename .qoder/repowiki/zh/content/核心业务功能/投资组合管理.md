...
## 更新摘要
**变更内容**
- 新增完整的业绩测量功能，包括时间加权收益率（TWR）和货币加权收益率（MWR）计算
- 添加风险指标计算，包括最大回撤和年化波动率等关键风险度量
- 实现标准化期间收益率，支持日、周、月、年等多周期收益分析
- 扩展投资组合API接口，提供全面的性能分析和风险评估能力
...
**重大更新** 投资组合管理系统现已集成完整的业绩测量功能，支持时间加权收益率（TWR）和货币加权收益率（MWR）计算，提供最大回撤、年化波动率等关键风险指标，以及标准化的期间收益率分析。这些功能为投资者提供了专业的投资绩效评估工具，支持多维度风险分析和收益归因。

## 项目结构
后端基于 FastAPI，采用分层组织：models（ORM 模型）、schemas（Pydantic 输入输出）、routers（路由与控制器）、services（业务服务，包含新增的性能服务）、dependencies（权限与安全）。前端通过 axios 封装统一 API 调用，类型定义与后端保持一致。
...
R["routers<br/>路由与控制器"]
P["services/performance_service.py<br/>业绩测量服务"]
...
FE_PERF["components/shared/PerformanceMetrics.tsx<br/>业绩指标组件"]
end
FE_API --> R
R --> M
R --> S
...
- [backend/app/services/performance_service.py:1-100](file://backend/app/services/performance_service.py#L1-L100)
...
- **新增** 业绩测量服务：提供TWR、MWR、风险指标和期间收益率计算。
- 路由与控制器：提供组合 CRUD、状态变更、净值历史、收益与现金流查询。
- Pydantic 模型：定义创建、更新、响应的数据结构。
- 权限依赖：用户认证与管理员权限校验。
- useNavHistory Hook：专门处理净值历史数据的获取和管理。
- NavCurve 图表组件：用于展示净值历史趋势图。

**重大更新** 新增的业绩测量服务是本次更新的核心功能，实现了完整的投资绩效分析能力。该服务支持多种收益率计算方法，包括时间加权收益率（TWR）用于评估基金经理表现，货币加权收益率（MWR）用于反映投资者实际收益，以及多种风险指标如最大回撤、年化波动率等。同时提供标准化的期间收益率计算，支持日、周、月、年等不同周期的收益分析。

章节来源
- [backend/app/models/portfolio.py:5-16](file://backend/app/models/portfolio.py#L5-L16)
- [backend/app/models/portfolio_position.py:5-52](file://backend/app/models/portfolio_position.py#L5-52)
...
- [backend/app/schemas/portfolio.py:6-30](file://backend/app/schemas/portfolio.py:6-L30)
- [backend/app/dependencies.py:114-146](file://backend/app/dependencies.py#L114-L146)
- [frontend/src/hooks/useNavHistory.ts:1-100](file://frontend/src/hooks/useNavHistory.ts#L1-L100)
- [frontend/src/components/charts/NavCurve.tsx:1-50](file://frontend/src/components/charts/NavCurve.tsx#L1-L50)

## 架构总览
后端通过 FastAPI 路由暴露组合管理接口，权限依赖负责鉴权与管理员校验。前端通过统一 API 封装发起请求，类型定义保证前后端一致性。新增的业绩测量服务通过专门的API接口提供性能分析功能。**新增** 净值历史数据获取流程，通过专门的 Hook 和图表组件实现时间序列数据的可视化展示。
...
participant PerfService as "performance_service.py"
participant DB as "数据库"
FE->>Hooks : "请求业绩测量数据"
Hooks->>API : "调用 getPortfolioPerformance"
API->>Router : "转发请求携带认证头"
Router->>PerfService : "调用业绩测量服务"
PerfService->>DB : "查询净值历史和交易数据"
DB-->>PerfService : "返回历史数据"
PerfService-->>Router : "计算并返回业绩指标"
Router-->>API : "序列化响应"
API-->>Hooks : "返回业绩测量结果"
Hooks-->>FE : "提供数据给业绩指标组件"
```
...
- [backend/alembic/versions/0002_investor_holding_derived_fields.py:21-31](file://backend/alembic/versions/0002_investor_holding_derived_fields.py#L21-L31)
...
### 业绩测量服务与收益率计算
**新增** 业绩测量服务是本次更新的核心功能，提供完整的投资绩效分析能力：

- **时间加权收益率（TWR）**：消除资金流入流出的影响，准确反映基金经理的投资管理能力
- **货币加权收益率（MWR）**：考虑资金流入流出的时机，反映投资者的实际收益表现
- **风险指标**：包括最大回撤、年化波动率、夏普比率等关键风险度量
- **标准化期间收益率**：支持日、周、月、年等不同周期的收益计算和比较
...
**重大更新** 新增业绩测量相关 API，提供完整的投资绩效分析能力：

- 组合业绩测量
  - 方法与路径：GET /api/portfolios/{code}/performance
  - 路径参数：code
  - 查询参数：start_date（可选）、end_date（可选）、benchmark（可选）
  - 返回：portfolio_code、twr_return、mwr_return、max_drawdown、annualized_volatility、sharpe_ratio、period_returns
  - 权限：普通用户

- 标准化期间收益率
  - 方法与路径：GET /api/portfolios/{code}/period-returns
  - 路径参数：code
  - 查询参数：period（日/周/月/年）、start_date、end_date
  - 返回：portfolio_code、period_type、returns、benchmark_returns
  - 权限：普通用户
...
**重大更新** 新增业绩测量功能支持更精确的投资绩效分析：
- TWR收益率：消除资金流影响，准确反映投资管理能力
- MWR收益率：考虑资金流时机，反映投资者实际收益
- 风险指标：最大回撤、年化波动率、夏普比率等全面风险度量
- 期间收益率：标准化日、周、月、年收益计算，支持基准对比
...
**重大更新** 业绩测量最佳实践：
- TWR收益率适用于评估基金经理表现，不受资金流影响
- MWR收益率反映投资者实际收益，考虑资金流时机
- 风险指标结合使用：最大回撤评估下行风险，波动率衡量价格波动，夏普比率评估风险调整后收益
- 期间收益率标准化：统一日、周、月、年收益计算，支持基准对比分析
- 数据修正最佳实践：发现数据错误时，优先使用 `POST /api/snapshots/recalculate` 进行区间重算
...
- **新增** 业绩测量服务依赖：performance_service.py 依赖净值快照数据和交易数据进行收益率计算。
...
PerfService --> NavModel
PerfService --> TradeModel
```

**重大更新** 新增业绩测量服务的依赖关系，performance_service.py 依赖净值快照和交易数据进行复杂的收益率和风险指标计算。**新增** 快照路由与事件监听器的依赖关系，所有日期字段统一使用snapshot_date，份额精度标准化通过 Alembic 迁移实现。**新增** 前端净值历史数据获取的依赖关系，useNavHistory Hook 作为中间层协调 API 调用和图表组件。
...
- **新增** 业绩测量计算优化：TWR和MWR计算采用增量算法，避免全量重算；风险指标计算使用向量化操作提升性能。
...
- **新增** 业绩测量计算错误：检查净值历史数据完整性，确保有足够的历史数据进行TWR/MWR计算；验证交易数据的时间顺序和连续性
- **新增** 风险指标计算错误：确认净值数据无缺失，检查极端值处理逻辑；验证基准数据可用性
...
**重大更新** 份额精度标准化是本次更新的核心改进，系统现在统一采用2位小数精度处理所有份额相关计算，确保财务计算的准确性和一致性。投资组合管理系统中导航历史契约标准化，date字段统一改为snapshot_date，提高了数据一致性。新增的 SQLAlchemy 事件监听器机制为持仓快照提供了强大的数据完整性保护，确保所有数据修正都通过标准化的重算流程进行，从根本上避免了数据不一致的风险。投资者持仓派生字段的增强提升了投资分析的深度和精度。**重大更新** 业绩测量功能的引入是本次更新的最大亮点，通过TWR、MWR、风险指标和标准化期间收益率的全面实现，为投资者提供了专业的投资绩效分析工具。useNavHistory Hook 和 NavCurve 图表组件的引入，为桌面端和移动端用户提供了完整的时间序列净值可视化能力，显著提升了投资分析体验。
...
- **新增** 业绩测量API：TWR、MWR、风险指标和期间收益率的接口说明和使用方法。
