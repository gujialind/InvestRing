...
- 快照服务（snapshot_service.py）
  ...
  - 自动确认申赎（当前未包含 Trade 自愈）
  - 硬闸门检查 pending 交易（存在 QDII 误杀问题）
  - 事件检查仅 warning（待改为拦截）
- 交易路由（trades.py）
  - 创建 Trade（一律 pending）
...
- 现金转移路由（cash_transfers.py）
  ...
  - 当天完成：两腿立即 confirmed
  - 跨天到账：卖出 leg 立即 confirmed，买入 leg pending，需显式确认
- 份额变动事件路由（share_change_events.py）
  - 确认事件时校验登记日持仓快照存在
  - 尚未在快照中应用事件（待实现）
- 可用资金计算（position_service.py）
  - 基于最新快照 + 未纳入快照的已确认申赎/调仓 + pending 买入扣减
  - 计划替换为 compute_implicit_flow，并区分实时预览与快照真相
- 现金手工更新（positions.py）
  - 当前直接 upsert portfolio_position.amount（违反“唯一真相源”原则，需改道 manual_market_value）
...
API->>DB : 卖出leg confirmed; 买入leg pending(confirm_date=None)
end
...
- auto_confirm_after_snapshot
  - 当前仅处理 Subscription（apply_date <= snapshot_date 的 pending 记录）
  - 需要扩展到 Trade（区分 confirm_trade 与 confirm_cash_transfer 两端点逻辑）
...
- 现状
  - 确认事件仅置 status=confirmed，不应用 shares_change/cash_change 到快照
  - 事件检查仅 warning，不拦截快照生成
- 应实现
  - 在快照生成时应用 ex_date <= target 的 confirmed 事件
  - pending 事件拦截快照生成
  - 双日期级联回退（ex_date 与 entitlement_date 任一被删快照即回退）
...
- 当天 vs 跨天
  - cross_day=False：两腿立即 confirmed
  - cross_day=True：卖出 leg confirmed，买入 leg pending（confirm_date=None）
- 前端交互
...
Cross --> |False| BothConfirmed["两腿立即confirmed"]
Cross --> |True| SellConfirmed["卖出leg confirmed"]
SellConfirmed --> BuyPending["买入leg pending(confirm_date=None)"]
BuyPending --> ConfirmLater["下一交易日确认"]
```
...
- 路由与服务耦合
  ...
  - trades.py 内部也实现了 _calculate_available_cash（与 position_service 重复，建议统一）
- 快照服务依赖
  - snapshot_service.py 依赖 trading_calendar、price_record、product 等基础数据
  - auto_confirm_after_snapshot 目前仅处理 Subscription，需扩展至 Trade
- 事件与快照
  - share_change_events.py 确认事件依赖 portfolio_position 快照存在性
  - 事件应用与级联回退需在 snapshot_service.py 中落地
...
- QDII T+2 被闸门误杀
  ...
  - 修复建议：Trade 分支改用 confirm_date <= target_date，并对 confirm_date is None 回退原逻辑
- 删快照→重生成卡死
  - 场景 B/C/D：跨天现金买入腿、QDII 在途、pending 事件导致 regen 无法自愈
  - 修复要点：闸门修复 + Trade 侧自愈 + 事件应用/级联
- 事件仅 warning 不拦截
  - 现象：pending 事件不阻止快照生成
  - 修复要点：将 _check_share_change_events 由 warning 改为 failed，并在生成阶段应用事件
