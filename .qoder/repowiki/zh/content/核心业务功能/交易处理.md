...
## 更新摘要
**变更内容**
- 新增在途资金模型架构，支持不对称跨日交易处理机制
- 买交易T日扣款，卖交易确认日或显式到达日扣款，消除负现金和NAV缺口窗口
- 实现了完整的在途资金跟踪系统，包括pending_cash_in和pending_cash_out字段
- 增强了现金可用性检查逻辑，考虑在途资金对可用余额的影响
- 优化了快照生成服务，支持在途资金的动态计算和状态管理
...
**更新重点**：本次更新引入了革命性的在途资金模型架构，彻底解决了传统交易系统中的负现金和NAV缺口问题。通过不对称跨日交易处理机制，买交易在T日立即扣款，卖交易在确认日或显式到达日扣款，确保了资金流的准确性和系统的稳定性。**特别重要的是，新的在途资金模型消除了交易过程中的资金缺口窗口，提供了更可靠的资金管理和风险控制能力**。

## 项目结构
交易处理功能主要由以下层次构成：
- 路由层：提供交易相关的 HTTP 接口，负责请求解析、权限校验与调用服务层。
- 服务层：封装数据访问与业务逻辑，包括净值计算、快照生成和统一的净值获取。**新增专门的交易服务模块和在途资金管理**。
- 模型层：定义交易、产品、持仓、订阅、交易日历与价格记录等核心实体及其约束。**新增在途资金字段和验证逻辑**。

**更新**：新增了完整的在途资金模型架构，支持不对称跨日交易处理和动态资金可用性检查，提供了更严格的资金验证和实时资金状态跟踪。
...
SSnapshot["服务: snapshot_service.py (增强)"]
SPos["服务: position_service.py"]
SSub["服务: subscription_service.py"]
SCash["服务: in_transit_cash_service.py (新增)"]
end
subgraph "模型层"
MTrade["模型: Trade (含DUPLICATE_TRADE约束)"]
MProduct["模型: Product"]
MPos["模型: PortfolioPosition (含在途资金字段)"]
...
- 持仓模型 PortfolioPosition：记录组合在某一日的持仓明细（份额、冻结份额、成本价、单位净值、市值、金额、冻结金额、快照日期），并包含"净值型/非净值型"的互斥约束与唯一索引。**支持按 platform_code 维度拆分，新增在途资金字段**。
...
- **新增**：在途资金服务 InTransitCashService：专门处理在途资金的跟踪、计算和管理，支持不对称跨日交易处理。
- **增强**：现金交易规范化机制：通过attach_paired_cash_leg辅助函数统一配对CASH腿的构建逻辑，确保所有CASH交易都包含完整的转账组信息，**新增CASH卖出操作的资金预留机制**。

**更新**：新增了完整的在途资金模型架构和服务层，支持不对称跨日交易处理和动态资金可用性检查，显著提升了系统的资金管理能力和风险控制水平。
...
- **新增**：在途资金管理：实时监控和处理在途资金，支持不对称跨日交易处理，消除负现金和NAV缺口窗口。

**更新**：新增了完整的在途资金模型架构，支持不对称跨日交易处理，买交易T日扣款，卖交易确认日或显式到达日扣款，彻底消除了负现金和NAV缺口窗口的问题。
...
participant IC as "服务 : in_transit_cash_service.py (新增)"
...
R-->>C : 返回确认结果
Note over ST,IC,DB : #37 使用 FOR UPDATE 锁和 savepoint 保证并发安全
```
...
### 在途资金模型架构（新增）
**新增功能**：实现了完整的在途资金模型架构，支持不对称跨日交易处理，彻底解决了传统交易系统中的负现金和NAV缺口问题。

#### 核心特性
- **不对称交易处理**：买交易T日立即扣款，卖交易在确认日或显式到达日扣款
- **在途资金跟踪**：实时跟踪pending_cash_in和pending_cash_out状态
- **动态可用性检查**：基于当前现金加上在途资金计算真实可用余额
- **NAV缺口消除**：通过精确的资金时点控制，消除交易过程中的资金缺口
- **跨日交易支持**：支持复杂的跨日交易场景和资金流转
- **实时状态同步**：在途资金状态与交易状态保持同步更新

```mermaid
flowchart TD
Start(["交易创建"]) --> CheckType{"交易类型?"}
CheckType --> |买入| DebitToday["T日立即扣款"]
CheckType --> |卖出| ConfirmDebit["确认日扣款"]
DebitToday --> UpdatePending["更新pending_cash_out"]
ConfirmDebit --> TrackPending["跟踪pending_cash_in"]
UpdatePending --> CalcAvailable["计算可用资金"]
TrackPending --> CalcAvailable
CalcAvailable --> ValidateEnough{"资金充足?"}
ValidateEnough --> |是| CreateOrder["创建交易订单"]
ValidateEnough --> |否| Reject["拒绝交易"]
CreateOrder --> End(["完成"])
Reject --> End
```
...
#### 资金流管理机制
- **T日扣款机制**：买入交易在T日立即从可用现金中扣除相应金额
- **确认日扣款机制**：卖出交易在确认日或显式到达日从账户中扣款
- **在途资金计算**：可用现金 = 当前现金 + pending_cash_in - pending_cash_out
- **状态同步**：交易状态变更时同步更新在途资金状态
- **快照集成**：快照生成时重新计算在途资金和可用余额
...
CalcShares --> UpdateInTransit["更新在途资金状态"]
UpdateInTransit --> UpdateStatus["更新状态为confirmed"]
...
**更新**：新增了skip_cash_check参数和在途资金集成，支持在特定场景下跳过现金检查，提高了系统的灵活性和适应性。
...
SyncAmount --> UpdateInTransit["更新在途资金"]
UpdateInTransit --> Complete(["完成创建"])
```
...
Success --> |是| UpdateInTransit["更新在途资金"]
Success --> |否| LogError["记录错误日志"]
UpdateInTransit --> LogSuccess["记录成功日志"]
...
Immediate --> UpdateInTransit["更新在途资金"]
Delayed --> UpdateInTransit
UpdateInTransit --> Complete["完成转移"]
```
...
CheckDuplicate --> CheckInTransit["检查在途资金影响"]
CheckInTransit --> CashCalc["_calculate_available_cash()"]
...
- **新增在途资金跟踪**：卖出交易在确认日扣款，跟踪pending_cash_in状态
- 成功后生成待确认订单。
...
ShareCalc --> TrackInTransit["跟踪在途资金"]
TrackInTransit --> Enough{"可用份额充足？"}
...
- **新增在途资金更新**：确认时同步更新在途资金状态

**更新**：确认流程新增了skip_cash_check参数、DUPLICATE_TRADE约束检查和在途资金更新，提高了系统的灵活性和数据安全性。
...
**图表来源**
- [backend/app/routers/trades.py:507-532](file://backend/app/routers/trades.py#L507-532)
...
- **新增在途资金结算**：根据交易类型和时点精确处理在途资金结算

**更新**：费用计算机制新增了CASH卖出资金预留、skip_cash_check参数支持和在途资金结算，提高了资金管理的准确性和灵活性。
...
- **新增在途资金计算**：考虑pending_cash_in和pending_cash_out对可用余额的影响
...
CheckReserve --> CheckInTransit["检查在途资金"]
CheckInTransit --> CalcPending["计算待确认交易影响"]
...
CheckDuplicate --> RecalcInTransit["重新计算在途资金"]
RecalcInTransit --> CalculateValue["计算市值"]
...
**更新**：确认接口新增了skip_cash_check参数和DUPLICATE_TRADE约束检查，提高了接口的灵活性和数据安全性。
...
- **新增在途资金风控**：考虑在途资金对可用余额的影响，提供更准确的资金可用性检查

**更新**：风控机制新增了CASH卖出资金预留、DUPLICATE_TRADE约束和在途资金风控，显著提升了系统的资金安全性和数据一致性。
...
- **新增在途资金查询**：支持按组合查询在途资金状态和历史变化

**更新**：批量处理和回测功能集成了DUPLICATE_TRADE约束检查和在途资金查询，提高了数据一致性和处理能力。
...
- 路由层依赖模型层与服务层：
  ...
  - **新增**：在途资金服务依赖Trade、PortfolioPosition进行在途资金跟踪和计算。
- 模型层之间通过外键与唯一约束建立强关联，确保数据一致性与业务规则落地。

**更新**：增强了交易服务和快照服务的依赖关系，新增了skip_cash_check参数支持、DUPLICATE_TRADE约束检查和在途资金服务依赖。
...
R --> P["模型: Product"]
R --> PP["模型: PortfolioPosition (含在途资金)"]
R --> S["模型: Subscription"]
...
ST --> P
ST --> PR
IC["服务: in_transit_cash_service.py (新增)"] --> T
IC --> PP
...
- **增强**：现金转移错误
  ...
  - **新增IN_TRANSIT_UPDATE_FAILED**：在途资金更新失败
- **增强**：并发相关错误
  - **CONCURRENT_MODIFICATION**：并发修改检测，建议使用重试机制
  - **LOCK_TIMEOUT**：数据库锁超时，检查长时间运行的事务
  - **新增DUPLICATE_TRADE_VIOLATION**：违反DUPLICATE_TRADE约束
  - **新增IN_TRANSIT_CONFLICT**：在途资金状态冲突
- **增强**：快照保护错误
  - **POSITION_UPDATE_BLOCKED**：尝试直接修改持仓快照被阻止
  - **SNAPSHOT_DEPENDENCY**：交易已被快照纳入，无法取消确认

**更新**：故障排查指南新增了CASH卖出资金预留失败、DUPLICATE_TRADE约束违反、在途资金更新失败等相关错误处理，提高了问题诊断能力。

**章节来源**
- [backend/app/routers/trades.py:298-336](file://backend/app/routers/trades.py#L298-336)
...
新增的DUPLICATE_TRADE约束确保了交易数据的唯一性，防止重复交易的发生。CASH卖出资金预留机制有效防止了账户透支风险。skip_cash_check参数为特定场景提供了灵活的现金检查控制选项。全面的并发安全机制和增强的错误处理进一步提升了系统的稳定性和可靠性。
...
- 术语
  ...
  - **新增在途资金模型**：支持不对称跨日交易处理的资金跟踪机制，包括pending_cash_in和pending_cash_out字段。
  - **新增不对称交易处理**：买交易T日扣款，卖交易确认日或显式到达日扣款的资金处理机制。
  - **新增NAV缺口消除**：通过精确的资金时点控制，消除交易过程中的资金缺口问题。
  - **新增在途资金服务**：专门处理在途资金跟踪、计算和管理的独立服务模块。
- 扩展建议
  ...
  - **新增在途资金监控**：监控在途资金状态变化和异常情况。
  - **新增不对称交易监控**：监控不对称跨日交易的处理情况和准确性。
  - **新增NAV缺口监控**：监控NAV缺口消除的效果和问题。
  - **新增在途资金报表**：生成在途资金状态和变化的分析报告。