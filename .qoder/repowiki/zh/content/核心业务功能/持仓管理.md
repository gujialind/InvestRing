...
## 更新摘要
**变更内容**
- **核心改进**：`update_cash_position` 函数现在对手动市值条目进行量化处理，确保手动调整的现金持仓保持统一的2位小数精度
- **精度控制**：引入 `quantize_amount` 函数，使用 ROUND_HALF_UP 四舍五入模式，防止浮点数误差累积
- **数据一致性**：所有手动现金调整金额统一量化为2位小数，与真实交易平台保持一致
- **测试覆盖**：新增完整的精度测试用例，验证量化处理的正确性
...
- **精度控制**：手动现金调整金额的量化处理，确保2位小数精度一致性
- 业务规则、计算公式与数据验证逻辑
...
## 项目结构
围绕持仓管理的关键文件组织如下：
- **服务层**：**核心重构后** position_service.py 提供显式现金余额计算和实时可用资金/份额计算，trading_utils.py 提供共享工具函数，quantize.py 提供精度量化工具
...
QU["quantize.py<br/>精度量化工具<br/>2位小数量化"]
...
- **核心重构** 显式现金余额计算服务（PositionService）
  ...
  - **精度控制** `update_cash_position`：现金市值修正，对手动调整金额进行量化处理，确保2位小数精度
- **手动市值覆盖服务**
  - 支持通过 `manual_market_value` 表进行绝对替换
  - 与显式现金余额计算结合，确保数据准确性
  - **新增精度控制**：所有手动调整金额统一量化为2位小数
- **共享交易工具函数**（TradingUtils）
  - `is_trading_day`：判断指定日期是否为交易日
  - `get_latest_snapshot_date`：获取组合最新快照日期
  - `get_next_trading_day`：获取下一个交易日
- **精度量化工具**（Quantize）
  - `quantize_amount`：将金额量化为2位小数（ROUND_HALF_UP，四舍五入）
  - `quantize_shares`：将份额量化为2位小数（ROUND_HALF_UP，四舍五入）
  - 统一精度标准，防止浮点数误差累积
...
- **手动市值覆盖**（ManualMarketValue）
  ...
  - **精度保证**：market_value 字段存储量化后的2位小数金额
...
## 架构总览
持仓管理采用"路由-服务-模型"的分层架构，**核心重构后引入快照基线算法、统一的手动市值覆盖机制和精度控制**：
- **服务层**：封装核心业务逻辑，包括显式现金余额计算、实时可用资金/份额计算、共享工具函数、精度量化工具
...
- **精度保障**：所有手动现金调整金额统一量化为2位小数，与真实交易平台保持一致
...
Router->>Service : update_cash_position()
Service->>Quantize : quantize_amount(amount)
Quantize-->>Service : 返回量化后的2位小数金额
Service->>DB : 检查组合和平台存在性
DB-->>Service : 返回验证结果
Service->>DB : 写入手动市值覆盖记录
ManualMMV->>DB : upsert记录量化后的金额
DB-->>ManualMMV : 提交成功
Service-->>Router : 返回成功响应
Router-->>Client : 返回量化后的金额
...
**图表来源**
- [backend/app/services/position_service.py:364-444](file://backend/app/services/position_service.py#L364-L444)
- [backend/app/utils/quantize.py:41-52](file://backend/app/utils/quantize.py#L41-L52)
- [backend/app/routers/positions.py:212-251](file://backend/app/routers/positions.py#L212-L251)
...
CheckTradingDay --> QuantizeAmount["量化金额到2位小数"]
QuantizeAmount --> ComputeValue["计算隐式现金值"]
...
### 组件C：精度量化工具
**精度量化工具模块**，提供统一的金额和份额量化处理：

#### 核心量化工具函数
- `quantize_amount(value)`：将金额量化为2位小数（ROUND_HALF_UP，四舍五入）
- `quantize_shares(value)`：将份额量化为2位小数（ROUND_HALF_UP，四舍五入）
- 统一精度标准，防止浮点数误差累积
- 负数语义：按绝对值四舍五入、远离零进位
...
```mermaid
flowchart TD
Input(["输入金额"]) --> CheckNone{"值为None？"}
CheckNone --> |是| ReturnNone["返回None"]
CheckNone --> |否| Convert["转换为Decimal"]
Convert --> Quantize["量化到2位小数<br/>ROUND_HALF_UP"]
Quantize --> Output["返回量化结果"]
```
...
HasManual --> |是| UseManual["使用手动市值覆盖值<br/>已量化2位小数"]
...
更新现金持仓，包含平台验证、交易日校验和数据库操作，使用新的手动市值覆盖机制和精度控制
...
### 组件G：组合持仓（PortfolioPosition）与投资人份额（InvestorHolding）
- 数据模型关系
...
+update_cash_position(db, portfolio_code, platform_code, amount, update_date, created_by) dict
}
...
- **核心重构** 服务层依赖
  ...
  - position_service.py 依赖 quantize.quantize_amount 精度量化工具
...
**图表来源**
- [backend/app/services/position_service.py:1-25](file://backend/app/services/position_service.py#L1-L25)
- [backend/app/services/trading_utils.py:1-14](file://backend/app/services/trading_utils.py#L1-L14)
- [backend/app/utils/quantize.py:1-25](file://backend/app/utils/quantize.py#L1-L25)
...
- **核心重构** 服务层问题排查
  ...
  - **精度控制**：验证 `quantize_amount` 函数是否正确处理金额量化
  - 确认数据库连接和会话管理是否正常
- 快照生成失败
  - 检查交易日历与组合状态
  - 检查是否存在待确认交易/申赎
...
**精度控制机制**确保了所有手动现金调整金额统一量化为2位小数，与真实交易平台保持一致，防止浮点数误差累积，提高了财务数据的准确性和可靠性。
...
- **核心重构**：calculate_available_cash 函数测试
  ...
  - 无快照时降级为全量流水计算
  - 覆盖日期不匹配时不影响可用现金
- **精度控制**：update_cash_position 函数测试
  - 金额量化为2位小数（ROUND_HALF_UP）
  - 1000.005 → 1000.01（四舍五入）
  - 1000.004 → 1000.00（向下舍入）
  - 负数金额正确处理
  - None值原样返回
...
- [backend/tests/integration/test_amount_precision.py:207-221](file://backend/tests/integration/test_amount_precision.py#L207-L221)