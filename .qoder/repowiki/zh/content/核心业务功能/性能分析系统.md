...
**所做更改**   
- 新增6个月、1年、3年收益期计算功能
- 使用relativedelta精确处理日期算术，支持闰年和月末边界情况
- 扩展前端展示组件以显示新的收益期指标
- 增强测试覆盖以验证新功能的正确性
...
- 绩效计算服务（performance_service.py）
  ...
  - **新增6个月、1年、3年收益期计算，使用relativedelta确保日期算术准确性**
...
**章节来源**
- [performance_service.py:1-305](file://backend/app/services/performance_service.py#L1-L305)
- [portfolio_value_snapshot.py:1-39](file://backend/app/models/portfolio_value_snapshot.py#L1-L39)
- [snapshot_service.py:144-237](file://backend/app/services/snapshot_service.py#L144-L237)
- [snapshot_recalc_job.py:27-66](file://backend/app/services/snapshot_recalc_job.py#L27-L66)
- [PerformanceMetrics.tsx:1-153](file://frontend/src/components/shared/PerformanceMetrics.tsx#L1-L153)
...
**图表来源** 
- [portfolios.py:136-147](file://backend/app/routers/portfolios.py#L136-L147)
- [performance_service.py:218-305](file://backend/app/services/performance_service.py#L218-L305)
- [PerformanceMetrics.tsx:70-153](file://frontend/src/components/shared/PerformanceMetrics.tsx#L70-L153)
...
- 设计要点
  ...
  - **新增收益期计算：6个月、1年、3年收益期，使用relativedelta确保日期算术准确性**
...
PeriodRet --> NewPeriods["计算6m/1y/3y收益期<br/>使用relativedelta"]
...
**章节来源**
- [performance_service.py:1-305](file://backend/app/services/performance_service.py#L1-L305)
- [test_performance_service.py:1-387](file://backend/tests/unit/test_performance_service.py#L1-L387)
...
- **新增功能说明**
  - 6个月收益期：以6个月前当日或之后首个快照净值为基准
  - 1年收益期：以1年前当日或之后首个快照净值为基准
  - 3年收益期：以3年前当日或之后首个快照净值为基准
  - 历史不足窗口期时显示"--"而非错误值

**章节来源**
- [PerformanceMetrics.tsx:1-153](file://frontend/src/components/shared/PerformanceMetrics.tsx#L1-L153)
- [portfolio.ts:55-89](file://frontend/src/types/portfolio.ts#L55-L89)
...
- 外部依赖
  ...
  - **新增：dateutil.relativedelta 用于精确日期算术**
...
- 计算复杂度
  ...
  - **新增收益期计算使用relativedelta，时间复杂度为O(1)**
- 数据访问
...
- **新增功能性能特点**
  - relativedelta 精确处理闰年和月末边界情况
  - 收益期计算与现有1m/3m/ytd计算保持一致的性能特征
...
- 区间重算冲突
  ...
  - 处理：等待任务完成或取消后再提交
- **新增收益期相关问题**
  - 现象：return_6m/return_1y/return_3y 为 null
  - 原因：组合成立时间不足相应窗口期
  - 处理：这是正常行为，表示历史数据不足以计算该收益期
...
## 结论
本性能分析系统以净值快照为核心，结合 TWR/MWR 双口径与风险指标，形成完整、可解释的组合绩效视图。**最新更新新增了6个月、1年、3年收益期计算功能，使用relativedelta确保日期算术的精确性，特别支持闰年和月末边界情况的正确处理**。通过严格的快照不可变性与级联回退机制，确保历史一致性；通过异步重算与可靠性提示，提升用户体验与数据可信度。建议在持有期较短时侧重累计收益与回撤，谨慎解读年化指标。

## 附录
- 前端类型契约（PortfolioPerformance）
  - 关键字段：twr、twr_chained、annualized_twr、mwr、initial_nav、current_nav、holding_days、return_1m、return_3m、**return_6m、return_ytd、return_1y、return_3y**、max_drawdown、max_drawdown_peak_date、max_drawdown_trough_date、annualized_volatility、cash_flow_count、nav_series_consistent、annualization_reliable
