...
## 项目结构
前端采用 Next.js App Router 架构，React Query 通过应用根组件 Provider 注入，各业务模块通过自定义 hooks 封装查询与变更逻辑，并在页面组件中消费。
...
subgraph "业务层"
Hooks["自定义 Hooks<br/>usePortfolio / usePosition / useProduct / useTrade / useInvestor"]
...
- [page.tsx（组合持仓页）:1-544](file://frontend/src/app/portfolio/[code]/positions/page.tsx#L1-L544)
- [page.tsx（平台管理页）:1-264](file://frontend/src/app/platforms/page.tsx#L1-L264)
...
- [useTrade.ts:23-46](file://frontend/src/hooks/useTrade.ts#L23-L46)
- [useInvestor.ts:15-31](file://frontend/src/hooks/useInvestor.ts#L15-L31)
...
## 结论
本项目通过在应用根部集中配置 QueryClient，并在各业务模块封装自定义 Hooks，实现了统一、可维护的数据获取与缓存策略。配合明确的查询键设计、精确的失效策略与完善的错误处理，整体具备良好的性能与一致性表现。建议在后续迭代中引入幂等更新与更细粒度的重试策略，进一步提升用户体验与系统稳定性。
