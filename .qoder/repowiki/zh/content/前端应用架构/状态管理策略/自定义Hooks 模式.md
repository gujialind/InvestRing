...
- 组合与快照 Hooks：usePortfolioList、usePortfolio、useCreatePortfolio、useUpdatePortfolio、useClosePortfolio、useActivatePortfolio、useDeletePortfolio、usePortfolioSnapshots、useLatestSnapshot、useAvailableCash、usePortfolioInvestors、持仓相关 Hooks
- 产品 Hooks：useProductList、useProduct、useCreateProduct、useUpdateProduct、useDeleteProduct
- 快照生成与重算 Hooks：useSnapshotStatus、useGenerateSnapshot、useRecalculateSnapshots、useValidateSnapshot、useDeleteSnapshot
- 交易与申赎 Hooks：useTradeList、useTrade、useCreateTrade、useUpdateTrade、useConfirmTrade、useCancelTrade、useBatchRebalance；申购赎回相关 Hooks
...
### 产品管理 Hooks（useProduct）
- useProductList/useProduct：列表/详情查询
- useCreateProduct/useUpdateProduct/useDeleteProduct：增删改并失效对应缓存
...
### 交易与申赎 Hooks（useTrade）
- 交易：useTradeList/useTrade、useCreateTrade/useUpdateTrade/useConfirmTrade/useCancelTrade、useBatchRebalance
- 申赎：useSubscriptionList/useSubscription、useCreateSubscription/useConfirmSubscription/useCancelSubscription
...
## 结论
本项目通过自定义 Hooks 将“数据获取/变更”“状态管理”“副作用”三大职责清晰分离，借助 React Query 实现高效缓存与请求控制，借助 Zustand Store 提供轻量全局状态，形成高内聚、低耦合的 Hooks 生态。遵循本文的模式与最佳实践，可在复杂金融业务中快速扩展新的 Hooks 并保持良好的可维护性与性能表现。
