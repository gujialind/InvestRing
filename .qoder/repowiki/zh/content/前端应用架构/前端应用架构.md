...
- [frontend/src/lib/api/cash-transfer.ts](file://frontend/src/lib/api/cash-transfer.ts)
...
## 更新摘要
**变更内容**
- 移除了项目根目录的 Node.js 包管理文件（package.json、package-lock.json）
- 前端开发环境设置文档需要更新以反映当前前端依赖和构建配置状态
- 所有前端相关的配置文件现在位于 frontend 子目录中
...
**更新** 项目结构已调整，Node.js 包管理文件已从项目根目录移除，所有前端相关配置现在集中在 frontend 子目录中。
...
- [frontend/src/app/globals.css:1-84](file://frontend/src/app/globals.css#L1-L84)
- [frontend/public/manifest.json:1-27](file://frontend/public/manifest.json#L1-L27)
...
章节来源
- [frontend/src/middleware.ts:1-40](file://frontend/src/middleware.ts#L1-L40)
- [frontend/src/lib/api/client.ts:1-94](file://frontend/src/lib/api/client.ts#L1-L94)
...
图表来源
- [frontend/src/lib/api/client.ts:1-94](file://frontend/src/lib/api/client.ts#L1-L94)
- [frontend/src/lib/api/cash-transfer.ts:1-26](file://frontend/src/lib/api/cash-transfer.ts#L1-L26)
...
图表来源
- [frontend/src/hooks/useCashTransfer.ts:20-44](file://frontend/src/hooks/useCashTransfer.ts#L20-L44)
- [frontend/src/lib/api/cash-transfer.ts:5-11](file://frontend/src/lib/api/cash-transfer.ts#L5-L11)
- [frontend/src/app/portfolio/[code]/positions/page.tsx:177-208](file://frontend/src/app/portfolio/[code]/positions/page.tsx#L177-L208)
...
- [frontend/src/app/portfolio/[code]/positions/page.tsx:441-537](file://frontend/src/app/portfolio/[code]/positions/page.tsx#L441-L537)
...
**更新** 项目结构已调整，Node.js 包管理文件已从项目根目录移除，所有前端相关的 package.json 和依赖管理现在位于 frontend 子目录中。
...
- **新增** 环境配置相关
  - 前端依赖安装失败：确认在 frontend 目录下执行 npm/yarn 命令。
  - 构建配置问题：检查 frontend/next.config.js 中的路径配置。

章节来源
- [frontend/src/lib/api/client.ts:30-42](file://frontend/src/lib/api/client.ts#L30-L42)
- [frontend/src/hooks/useAuth.ts:1-134](file://frontend/src/hooks/useAuth.ts#L1-L134)
- [frontend/src/middleware.ts:1-40](file://frontend/src/middleware.ts#L1-L40)
- [frontend/next.config.js:5-12](file://frontend/next.config.js#L5-L12)
- [frontend/src/stores/authStore.ts:1-71](file://frontend/src/stores/authStore.ts#L1-L71)
- [frontend/src/stores/uiStore.ts:1-85](file://frontend/src/stores/uiStore.ts#L1-L85)
- [frontend/src/app/globals.css:29-49](file://frontend/src/app/globals.css#L29-L49)
- [frontend/src/app/layout.tsx:8-17](file://frontend/src/app/layout.tsx#L8-L17)
- [frontend/src/hooks/useCashTransfer.ts:36-43](file://frontend/src/hooks/useCashTransfer.ts#L36-L43)

## 结论
InvestRing 前端以 Next.js 14+ App Router 为核心，结合中间件实现移动端/PC 端分流与统一鉴权；通过 Zustand 与 React Query 构建清晰的状态与数据流；UI 层以 shadcn/ui + TailwindCSS 提供一致的视觉与交互体验。**新增的现金转移功能**完善了平台间资金管理的能力，支持灵活的转账模式和完善的错误处理。**项目结构已优化**，Node.js 包管理文件已从项目根目录移除，所有前端相关配置现在集中在 frontend 子目录中，提高了项目的模块化程度和维护性。建议后续完善 Service Worker 与缓存策略，进一步增强 PWA 能力与离线可用性；持续优化数据缓存与图表渲染性能，提升复杂场景下的用户体验。
...
### 开发环境设置指南
- **更新** 项目结构已调整，Node.js 包管理文件已从项目根目录移除
- 前端开发环境现在位于 frontend 子目录中
- 安装依赖：在 frontend 目录下执行 `npm install` 或 `yarn install`
- 启动开发服务器：在 frontend 目录下执行 `npm run dev`
- 构建生产版本：在 frontend 目录下执行 `npm run build`
- 环境变量配置：在 frontend 目录下创建 `.env.local` 文件
- Docker 部署：使用 frontend/Dockerfile 进行容器化部署
