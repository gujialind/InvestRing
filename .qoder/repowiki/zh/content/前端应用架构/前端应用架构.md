...
章节来源
- [src/middleware.ts:1-40](file://frontend/src/middleware.ts#L1-L40)
- [src/lib/api.ts:1-627](file://frontend/src/lib/api.ts#L1-L627)
...
- 按业务域拆分 API 模块（认证、投资人、组合、持仓、交易、产品、平台、系统、日志与任务、快照、份额变动事件等），便于维护与测试。
...
章节来源
- [src/lib/api.ts:67-79](file://frontend/src/lib/api.ts#L67-L79)
- [src/hooks/useAuth.ts:1-134](file://frontend/src/hooks/useAuth.ts#L1-L134)
- [src/middleware.ts:1-40](file://frontend/src/middleware.ts#L1-L40)
- [next.config.js:5-12](file://frontend/next.config.js#L5-L12)
- [src/stores/authStore.ts:1-71](file://frontend/src/stores/authStore.ts#L1-L71)
...
## 结论
InvestRing 前端以 Next.js 14+ App Router 为核心，结合中间件实现移动端/PC 端分流与统一鉴权；通过 Zustand 与 React Query 构建清晰的状态与数据流；UI 层以 shadcn/ui + TailwindCSS 提供一致的视觉与交互体验。建议后续完善 Service Worker 与缓存策略，进一步增强 PWA 能力与离线可用性；持续优化数据缓存与图表渲染性能，提升复杂场景下的用户体验。
