...
## 核心组件
- Tailwind 配置与内容扫描路径：通过 content 字段限定扫描范围，确保按需生成样式，减少体积。
...
**章节来源**
- [tailwind.config.ts:3-54](file://frontend/tailwind.config.ts#L3-L54)
- [globals.css:5-59](file://frontend/src/app/globals.css#L5-L59)
...
## 架构总览
样式与主题系统围绕“变量驱动 + 原子化组件 + 状态管理”的架构展开：
- 变量驱动：CSS 变量集中于根作用域，Tailwind 通过 hsl(var(--x)) 引用，实现主题切换。
- 原子化组件：按钮、输入框、卡片等组件以最小语义单元组合，支持变体与尺寸。
- 状态管理：UI 状态（如侧边栏、Toast）通过 Zustand 存储，影响布局与交互反馈。
- 构建流水线：PostCSS + TailwindCSS 生成最终样式，按需裁剪。

```mermaid
graph TB
ROOT[":root<br/>CSS 变量"] --> THEME["主题变量<br/>hsl(var(--*))"]
...
**图表来源**
- [globals.css:5-49](file://frontend/src/app/globals.css#L5-L49)
...
### Tailwind 配置与定制化策略
- 内容扫描：限定扫描 pages、components、app 目录，避免无用样式进入产物。
- 主题扩展：colors 与 borderRadius 通过 CSS 变量桥接，实现与 CSS 变量一致的主题控制。
- 插件：当前未启用插件，保持构建简洁；如需新特性可按需引入。

最佳实践
- 仅在 content 中声明实际使用的目录，避免样式膨胀。
- 使用 CSS 变量作为单一真实来源，Tailwind 仅做令牌映射。
...
- Tailwind 引用：通过 hsl(var(--x)) 在 theme.extend.colors 中引用变量，保证一致性。
...
**章节来源**
- [globals.css:6-26](file://frontend/src/app/globals.css#L6-L26)
- [globals.css:29-49](file://frontend/src/app/globals.css#L29-L49)
- [tailwind.config.ts:11-50](file://frontend/tailwind.config.ts#L11-L50)
...
- 通过扩展 tailwind.config.ts 的 theme.extend，新增自定义令牌，再在组件中使用。
...
- 基础层：globals.css 的 @layer base 与 @layer utilities 提供全局基线与通用工具类。

**章节来源**
- [button.tsx:6-33](file://frontend/src/components/ui/button.tsx#L6-L33)
- [utils.ts:8-10](file://frontend/src/lib/utils.ts#L8-L10)
- [globals.css:52-72](file://frontend/src/app/globals.css#L52-L72)
...
## 依赖关系分析
- 构建链路：package.json 中声明 tailwindcss、postcss、autoprefixer，postcss.config.js 指定插件顺序。
- 组件依赖：button/input/card 组件依赖 cn 工具与 CSS 变量；布局依赖全局样式；Toast 依赖 uiStore。
- 主题耦合：Tailwind 配置与 globals.css 的变量定义强关联，任一改动会影响组件外观。
...
## 性能考量
- 按需生成：content 限定扫描目录，避免无关文件参与样式生成。
...
- 样式不生效
  ...
  - 确认 Tailwind 配置的 content 路径包含目标组件目录。
- 主题切换无效
...
- 构建异常
  - 检查 postcss.config.js 中插件顺序与版本兼容性。
...
## 结论
本项目采用“CSS 变量 + Tailwind 令牌 + 原子化组件”的样式体系，具备良好的主题一致性与可维护性。通过 cn 工具与变体系统，组件样式复用与定制化得到平衡；借助 Zustand 管理 UI 状态，Toast 与布局反馈清晰可控。建议在后续迭代中：
- 明确主题切换入口与持久化策略
- 扩展 tailwind.config.ts 以支持更多自定义令牌
- 逐步沉淀通用工具类至 @layer utilities
