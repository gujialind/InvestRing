...
采用逐日 checkpoint commit/rollback，是 backend/AGENTS.md §1.1「service 不 commit」
...
def generate_daily_snapshots(
    ...
    Note:
        本函数不 commit/rollback（backend/AGENTS.md §1.1，事务边界交调用方），
...
def recalculate_snapshots(
    ...
    Note:
        本函数全程不 commit/rollback（issue #58，backend/AGENTS.md §1.1）：
...
def catch_up_snapshots(
    ...
    （编排层 checkpoint，backend/AGENTS.md §1.1 例外）；单日失败 rollback 当日并停止，
...
def generate_next_snapshot(
    ...
    生成 + auto_confirm 后即 commit（编排层 checkpoint，backend/AGENTS.md §1.1 例外）。
...
def list_portfolio_snapshots(
    ...
    不 commit、不抛 HTTPException（分层约定见 backend/AGENTS.md §1.1）。
    """
