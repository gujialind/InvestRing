...
* **PUT 直改与创建同口径**（#182）：编辑 pending 交易时 amount/actual_amount 均视为实际金额（buy=含费现金支出、sell=到手净额，`actual_amount` 优先），service 层联动重算净额列并镜像 CASH 腿；改金额/份额/日期实时校验可用量（加回自身 pending 旧值）、非交易日直接拒绝不静默滚交易日、自然键防重排除自身（无 `allow_duplicate`）；CASH 腿仅 notes 放行；校验全部通过前零写入。
