...
export default function SettingsPage() {
  ...
  const { data: calendarData, isLoading: calendarLoading } = useQuery({
    queryKey: ["trading-calendar", syncYear],
    queryFn: () => systemApi.getTradingCalendar(syncYear),
  });

  const { data: dataSourceData, isLoading: dataSourceLoading } = useQuery({
  ...
  const syncCalendar = useMutation({
    ...
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries({ queryKey: ["trading-calendar", syncYear] });
      addToast({
        type: "success",
        title: "同步成功",
  ...
  const updateDataSource = useMutation({
    mutationFn: (data: { source: string; config: Record<string, string> }) =>
      systemApi.updateDataSourceConfig(data),
    onSuccess: () => {
      addToast({
        type: "success",
        title: "保存成功",
  ...
  const handleSaveDataSource = () => {
    updateDataSource.mutate({
      ...
      config: {
        ...
        akshare_enabled: String(dataSourceConfig.akshare_enabled),
      },
    });
  };
