...
export default function ProductsPage() {
  ...
  const updateProduct = useMutation({
    mutationFn: ({ code, data, market }: { code: string; data: ProductUpdate; market?: string }) => productApi.update(code, data, market),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["products", "list"] });
  ...
  const { data: priceData, isLoading: priceLoading } = useQuery({
    queryKey: ["product-prices", selectedProduct?.code, selectedProduct?.market],
    queryFn: () =>
      selectedProduct && selectedProduct.market
        ? productApi.getPriceData(selectedProduct.code, selectedProduct.market, { limit: 30 })
        : Promise.resolve([]),
    enabled: !!selectedProduct && !!selectedProduct.market && priceDialogOpen,
    staleTime: 5 * 60 * 1000,
  });
